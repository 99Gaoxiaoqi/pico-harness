...
> 这是本机 Codex 的用户偏好覆盖规则，优先级高于下面的简版输出示例。

每次通过 AI HOT API 拿到条目后，**不能只转述 API 里的标题和 summary**。对最终展示给用户的每一则 AI news，都必须做三件事：

1. **追溯并核对最原始的数据源**：优先打开条目里的 `url` / `sourceUrl`。如果它指向聚合页、转载页、二手报道或无法访问，就继续寻找对应的官方博客、论文页、GitHub release、产品公告、监管文件、公司新闻稿、创始人/研究员原帖等一手来源。输出中的链接使用这个最原始来源；若暂时无法确认一手来源，明确标注"原始来源未能核实"，不要把 AI HOT 摘要当作引用依据。
2. **深度解析数据源**：分析这个来源本身在说什么、没说什么、为什么现在说、谁在说、证据强度如何。区分官方确认、研究论文、产品文档、媒体报道、X 帖、截图、二手转述、营销话术。不要把来源里的宣传口径直接当结论。
3. **逐条写深层含义**：每条都补"深度含义"。不要泛泛说"值得关注"，而是说明它可能改变 AI 界的哪一层：模型能力、产品形态、开发者生态、算力/成本、监管与版权、开源竞争、科研方向、创业机会、分发入口、用户习惯或权力结构。必须尽量给出二阶影响：这件事如果是真的，下一步谁会被迫反应，谁会受益，谁会承压，哪些旧假设会失效。

### 深度解析要求

每条新闻默认至少覆盖以下 4 点；如果篇幅紧张，可以合并成短段，但不能只写表层摘要：

- **事实层**：原始来源能确认的事实是什么，哪些只是推测或截图证据。
- **来源层**：发布者是谁，可能的动机是什么，可信度和局限在哪里。
- **结构层**：这件事落在 AI 产业链的哪一环，和模型、产品、算力、渠道、监管、开源生态的关系是什么。
- **含义层**：它的深层含义和二阶影响是什么。写具体判断，不写"有待观察"这类空话；不确定时说明不确定来自哪里。

### X / Twitter 来源处理

当条目来源是 X/Twitter，或 `url` / `sourceUrl` 指向 `x.com`、`twitter.com` 时，**不要用普通网页抓取或 curl 作为首选**。优先使用 OpenCLI 的 Twitter 适配器读取：

```bash
opencli twitter thread "<tweet-url-or-id>" --limit 5 -f json
```

核对时至少看这些字段：`text`、`created_at`、`author`、`url`、`quoted_tweet`、`card`、`media_urls`。很多 X 新闻把关键证据放在截图里；如果 `has_media=true`，需要下载并查看 `media_urls`，必要时做 OCR / 人工读图。若事实只存在于截图或转述推文中，输出时要说清楚这是"X 帖/截图显示"，不要提升成官方确认。若 `quoted_tweet` 指向更原始发布者，优先继续追溯 quoted tweet。

默认只输出已经完成原始来源核对的条目。若用户要求的条目很多，可以先筛选最重要的 5-10 条完整核对；如果用户明确要全量，就按批次处理，不能把未核对来源的条目混在已核对结论里。
...
1. **<标题>** — <原始来源名>
   <一句话事实摘要，来自原始来源核对后改写>
   数据源解析：<来源性质、可信度、动机或局限>
   深度含义：<这件事对 AI 界的二阶影响和可能改变的结构>
   原始来源：<url>
```
