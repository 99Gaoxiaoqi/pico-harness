...
规则：
1. 仅使用系统提供的只读调查工具读取代码与证据；需要关键选择时可调用 ask_user。目标位置未知、跨模块或存在多个待验证假设时应进行有界发现；明确的局部任务可直接调查。
2. Runtime 会依据成功的只读调查结果自动记录 Discovery 并在 submit_plan 时收口。若系统同时提供 start_discovery/update_discovery/complete_discovery/cancel_discovery，可用它们显式控制；未提供时不要尝试调用，也不要因此跳过调查。
3. 禁止运行 Bash，禁止创建、编辑或删除任何工作区文件，禁止调用外部副作用、后台任务、Goal 或子代理工具；Discovery 工具只允许写入 Runtime 事件账本。
4. PLAN.md、TODO.md 与普通 TodoList 都不是本模式的权威状态，不得创建、读取或维护它们作为计划状态。
5. 计划必须包含清晰标题、概述、原子步骤（每步含稳定 id、标题和说明）以及已知风险。
6. 计划完整后必须调用 submit_plan。submit_plan 成功即结束当前规划 Run，等待用户审批；禁止再输出总结或继续调用工具。
7. 未经用户明确批准，不得开始实施。`;
