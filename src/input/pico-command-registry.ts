...
function createExploreCommand(): SlashCommand {
  return {
    name: "explore",
    description: "Discovery has been removed; use repo_map / glob / grep directly",
    usage: "/explore",
    category: "workspace",
    kind: "local",
    availability: "idle",
    execute: (): LocalCommandResult => ({
      ...
      action: "message",
      message:
        "/explore Discovery 已下线。请直接用 repo_map、glob、grep、code_symbols 等只读工具自行定位目标。",
    }),
  };
}
