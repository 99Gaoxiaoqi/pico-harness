...
export class SqliteRuntimeEventStore {
  ...
  private readonly warningLogger: RuntimeEventStoreOptions["warningLogger"];
   ...
   * 投影消费方(压缩读模型、plan/graph 折叠)以本方法替代
...
export async function appendRuntimeEventBatchWithArbitration(
  ...
  try {
    ...
    store.warnAmbiguousAppendRecovered(events.map((event) => event.eventId));
    return recovered;
  }
}
...
async function arbitrateDurableAppendFailure(
  ...
  for (const event of events) {
    ...
    const transcriptWatermark = watermarks.get(event.sessionId);
    if (!transcriptWatermark) return undefined;
    results.push({
...
/** 读取既有会话的完整投影；存储根缺失时不创建并返回 undefined。 */
