...
import { safeAgentGraphErrorMessage } from "@pico/core/agent-graph-diagnostics";
...
export * from "@pico/core/agent-graph-store-contracts";
...
function normalizeResourceRefInput(
  ...
  const mediaType = optionalNonEmpty(input.mediaType, "mediaType");
...
type JsonColumnValue =
  ...
  | { readonly [key: string]: JsonColumnValue };

function rowJson(row: Record<string, unknown>, key: string): JsonColumnValue {
  return JSON.parse(rowString(row, key)) as JsonColumnValue;
}
