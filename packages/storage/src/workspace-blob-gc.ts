...
export interface RunWorkspaceBlobGcOptions {
  readonly paths: WorkspaceBlobGcPaths;
  readonly limit?: number;
  readonly now?: () => Date;
}

/** Storage-owned directories required to drain a workspace Blob GC outbox. */
export interface WorkspaceBlobGcPaths {
  readonly workspaceRoot: string;
  readonly workspaceEvidenceDirectory: string;
  readonly homeFileHistoryDirectory: string;
  readonly homeWorkspacesDirectory: string;
}
...
export async function runWorkspaceBlobGcOnce(
  ...
  try {
    lease = await OwnerLease.acquire({
      leaseDirectory: join(paths.workspaceRoot, ".leases", "blob-gc"),
  ...
  let retryable = 0;
  try {
    const intents = readPendingIntents(paths.workspaceRoot, limit, startedAt);
    for (const intent of intents) {
      await lease.assertOwnership();
      processed += 1;
      try {
        await applyIntent(intent, paths, lease);
        recordIntentResult(paths.workspaceRoot, intent, { status: "completed" });
        completed += 1;
      } catch (error) {
        recordIntentResult(paths.workspaceRoot, intent, {
          status: "retryable",
  ...
  const schedule = readPendingIntentSchedule(paths.workspaceRoot, options.now?.() ?? new Date());
  return { status: "completed", processed, completed, retryable, ...schedule };
}

async function applyIntent(
  intent: BlobGcIntent,
  paths: WorkspaceBlobGcPaths,
  ...
  if (intent.kind === "file_history") {
    await withFileHistoryMutationLease(
      paths.homeFileHistoryDirectory,
      `blob-gc:${process.pid}:${intent.intentId}`,
      async (mutationLease) => {
        await workspaceLease.assertOwnership();
        await mutationLease.assertOwnership();
        if (fileHistoryDigestIsReferenced(paths.homeWorkspacesDirectory, intent.digest)) {
          throw new Error(`File History blob ${intent.digest} is still referenced`);
        }
        if (!isFileHistoryMutationLeaseHeld(paths.homeFileHistoryDirectory)) {
          throw new Error("File History mutation lease was lost before unlink");
        }
        await unlinkVerifiedCasBlobIfPresent(paths.homeFileHistoryDirectory, intent);
      },
  ...
  if (intent.kind === "evidence") {
    if (
      ...
      workspaceDigestIsReferenced(paths.workspaceRoot, "evidence", intent.digest)
    ) {
      throw new Error(`Evidence blob ${intent.digest} is still referenced`);
    }
    try {
      await withVerifiedEvidenceDirectory(
        paths.workspaceEvidenceDirectory,
  ...
  const relativeAssetPath = containedRelativePath(paths.workspaceRoot, assetPath);
