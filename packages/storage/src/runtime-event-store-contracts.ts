...
import type { DurableTranscriptEvent, SessionCursor } from "@pico/core";
...
export const RUNTIME_TRANSCRIPT_PROJECTOR_VERSION = 5 as const;
...
export interface RuntimeEventStoreOptions {
  ...
  /** Optional host diagnostics for ambiguous-write readback recovery. */
  readonly warningLogger?: {
...
export interface PrepareRuntimeToolOperationInput<
  TTranscriptEvent = DurableTranscriptEvent,
> extends RuntimeFencedWriteOptions {
  readonly providerEvents?: readonly RuntimeEvent<TTranscriptEvent>[];
  readonly dispatchEvent: Extract<RuntimeEvent<TTranscriptEvent>, { kind: "tool.started" }>;
  readonly toolCallId: string;
  readonly providerCallId?: string;
}
...
export interface SettleRuntimeToolOperationInput<
  TTranscriptEvent = DurableTranscriptEvent,
> extends RuntimeFencedWriteOptions {
  readonly resultEvent: Extract<RuntimeEvent<TTranscriptEvent>, { kind: "tool.result.recorded" }>;
  readonly toolCallId: string;
  readonly expectedVersion: number;
}
...
export interface RuntimeEventStoreEntry<TTranscriptEvent = DurableTranscriptEvent> {
  readonly sequence: number;
  readonly event: RuntimeEvent<TTranscriptEvent>;
}

/** One workspace session read in a batched store-level listing pass. */
export interface WorkspaceRuntimeSessionSnapshot<TTranscriptEvent = DurableTranscriptEvent> {
  readonly manifest: RuntimeSessionManifest;
  readonly entries: readonly RuntimeEventStoreEntry<TTranscriptEvent>[];
}

export interface RuntimeSessionProjectionSnapshot<TTranscriptEvent = DurableTranscriptEvent> {
  readonly manifest: RuntimeSessionManifest;
  readonly entries: readonly RuntimeEventStoreEntry<TTranscriptEvent>[];
  readonly cursor?: SessionCursor;
}

export interface RuntimeSessionProjectionDelta<TTranscriptEvent = DurableTranscriptEvent> {
  readonly entries: readonly RuntimeEventStoreEntry<TTranscriptEvent>[];
  readonly cursor: SessionCursor;
}
 ...
 */
export interface StartRuntimeContinuationInput<
  _TTranscriptEvent = DurableTranscriptEvent,
> extends RuntimeFencedWriteOptions {
  readonly sessionId: string;
  readonly sourceRunId: string;
  readonly targetRunId: string;
...
export type RuntimeContinuationStartOutcome<TTranscriptEvent = DurableTranscriptEvent> =
  | {
      readonly status: "started" | "replayed";
      readonly claim: RuntimeContinuationClaim;
      readonly startEvent: Extract<RuntimeEvent<TTranscriptEvent>, { kind: "run.started" }>;
