...
/** @deprecated Fork bootstrap run-id namespace is a Core durable protocol contract. */
export { RUNTIME_FORK_BOOTSTRAP_RUN_PREFIX };
...
export function foldSessionSummaryEvent<TTranscriptEvent>(
  ...
  if (event.kind === "session.state.committed") {
    const settings = event.data.patch.settings;
    // settings 是整体替换：最后一份对象决定 title/forkFrom，缺省字段即清除。
    if (settings?.title !== undefined) next.settingsTitle = settings.title;
    else delete next.settingsTitle;
    if (settings?.forkFrom !== undefined) next.settingsForkFrom = settings.forkFrom;
    else delete next.settingsForkFrom;
  }
...
export function summaryFromRuntimeSession<TTranscriptEvent>(
  manifest: RuntimeSessionManifest,
  entries: readonly RuntimeEventStoreEntry<TTranscriptEvent>[],
): SequencedCliSessionSummary {
...
export function computeSessionPublicationFlags<TTranscriptEvent>(
  entries: readonly RuntimeEventStoreEntry<TTranscriptEvent>[],
