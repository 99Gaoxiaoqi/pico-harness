...
/** Product logging remains an outer concern; Runtime only needs warning delivery. */
...
export interface CompactorOptions {
  ...
  /** Optional product logger; omitted reusable Runtime instances stay silent. */
...
export class Compactor {
  ...
  compact(msgs: Message[]): Message[] {
    ...
    if (this.runtimeState.ineffectiveCount >= 2) {
      this.logger.warn("[Compactor] 连续压缩收益不足,本轮跳过压缩以避免反复抖动。");
      return sanitizeToolPairs(msgs);
    }
