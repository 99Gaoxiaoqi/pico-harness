...
/** Process-local task lifecycle and presentation projection owned by Runtime. */
export class TaskRegistry {
  ...
  private update(
    ...
    const data = mergeData(task.snapshot.data, input.data);
    task.snapshot = {
      ...task.snapshot,
      status,
      ...defined({
...
function cloneSnapshot(snapshot: TaskSnapshot): TaskSnapshot {
  ...
  return { ...snapshot, ...(data === undefined ? {} : { data }) };
}
