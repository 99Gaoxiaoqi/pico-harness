...
export interface RuntimeTaskMirrorOptions {
  ...
  logger?: RuntimeTaskMirrorLogger;
}

export interface RuntimeTaskMirrorLogger {
  error(bindings: object, message: string): void;
  warn(bindings: object, message: string): void;
}

const NOOP_LOGGER: RuntimeTaskMirrorLogger = {
  error: () => undefined,
  warn: () => undefined,
};
...
/**
 * Compatibility bridge while Background and Worktree still publish TaskRegistry snapshots.
...
export class RuntimeTaskMirror {
  ...
  private readonly logger: RuntimeTaskMirrorLogger;
  ...
  private mirror(snapshot: TaskSnapshot): void {
    ...
    try {
      ...
      if (!durable) {
        const ownerSessionId = stringData(snapshot, "ownerSessionId");
        const childSessionId = stringData(snapshot, "childSessionId");
        this.jobs.dispatch({
...
export function materializeRuntimeTaskSnapshot(durable: JobWithAttempts): TaskSnapshot {
  const { job } = durable;
  const attempt = durable.attempts.at(-1);
  const outputFile = attempt?.outputPath ?? job.outputPath;
  const error = job.error ?? attempt?.error;
