import { deterministicFingerprint } from "@pico/core/agent-graph-identities";
...
import { SqliteAgentGraphControlStoreAdapter } from "./agent-graph-control-store-adapter.js";
import {
  projectAgentGraphRuntimeActivation,
  type AgentGraphRunLaunchState,
} from "./agent-graph-runtime-activation-projection.js";

/** Read-only Runtime ledger access required by the Graph desktop projection. */
export interface AgentGraphRuntimeEventQueryPort {
  readRun(
    sessionId: string,
    runId: string,
  ): Promise<readonly AgentGraphRuntimeEventEnvelope[]> | readonly AgentGraphRuntimeEventEnvelope[];
  readEvent(
    eventId: string,
  ):
    | Promise<AgentGraphRuntimeEventEnvelope | undefined>
...
/**
 * Read model for Graph commands, control facts and durable Runtime/output facts.
 *
 * Storage remains the authority for Graph control rows. The Runtime ledger is
 * deliberately injected as a decoded read port: this package must not know the
 * host's SQLite event schema or feature-specific event decoder.
 */
export class AgentGraphReadOnlyQueryService {
  ...
  async queryRuntimeFacts(
    graphId: string,
    runtimeEvents: AgentGraphRuntimeEventQueryPort,
    ...
    const runtimeClaims = await Promise.all(
      claims.map(async (claim) => {
        const [events, launchState] = await Promise.all([
          runtimeEvents.readRun(claim.targetSessionId, claim.targetRunId),
    ...
    const outputs = await Promise.all(
      this.store.listRecordRefs(graphId).map(async (record) => {
        const event = await runtimeEvents.readEvent(record.sourceEventId);
        const claim = claimsById.get(record.claimId);
        const status = agentOutputStatus(event);
        if (
          !event ||
          !claim ||
          status === undefined ||
          ...
          !hasMatchingActivationId(event, claim.claimId)
        ) {
...
function agentOutputStatus(
  event: AgentGraphRuntimeEventEnvelope | undefined,
): "success" | "failure" | undefined {
  if (!event || event.kind !== "agent.output") return undefined;
  const data = event.data as { readonly payload?: unknown };
  if (!isRecord(data.payload)) return undefined;
  const status = data.payload.status;
  return status === "success" || status === "failure" ? status : undefined;
}

function hasMatchingActivationId(event: AgentGraphRuntimeEventEnvelope, claimId: string): boolean {
  if (event.kind !== "agent.output") return false;
  const data = event.data as { readonly payload?: unknown };
  return isRecord(data.payload) && data.payload.activationId === claimId;
}
