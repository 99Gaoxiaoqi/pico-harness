...
import type { SessionTaskRecord, SessionTaskStatus } from "@pico/storage";
...
export interface SessionTaskRepositoryPort {
  createTask(input: {
  ...
  }): { readonly revision: number; readonly tasks: readonly SessionTaskRecord[] };
}

export interface BoundSessionTaskAuthority {
  readonly repository: SessionTaskRepositoryPort;
  readonly sessionId: string;
  readonly onChanged?: (revision: number) => void;
  /** Host-specific observability; mutation already committed when this runs. */
  readonly onObserverError?: (input: {
    readonly sessionId: string;
...
export interface SessionTaskToolExecutionContext {
  readonly toolCallId?: string;
}

export interface SessionTaskTool {
  readonly fileSideEffects: typeof NO_FILE_SIDE_EFFECTS;
  readonly toolset: "session-tasks";
  readonly readOnly: boolean;
  readonly permissionCategory?: "bounded_control";
  name(): string;
  definition(): ToolDefinition;
  execute(args: string, context?: SessionTaskToolExecutionContext): Promise<string>;
  accesses(args: string): ToolAccessesValue;
}

abstract class BaseSessionTaskTool implements SessionTaskTool {
  readonly fileSideEffects = NO_FILE_SIDE_EFFECTS;
  readonly toolset = "session-tasks" as const;
  ...
  abstract execute(args: string, context?: SessionTaskToolExecutionContext): Promise<string>;

  accesses(): ToolAccessesValue {
...
export class SessionTaskCreateTool extends BaseSessionTaskTool {
  ...
  async execute(args: string, context?: SessionTaskToolExecutionContext): Promise<string> {
...
export class SessionTaskUpdateTool extends BaseSessionTaskTool {
  readonly readOnly = false;
  readonly permissionCategory = "bounded_control" as const;
...
export class SessionTaskGetTool extends BaseSessionTaskTool {
  ...
  async execute(args: string, _context?: SessionTaskToolExecutionContext): Promise<string> {
    const input = this.parse(args);
...
export function createSessionTaskTools(
  authority: BoundSessionTaskAuthority,
): readonly SessionTaskTool[] {
  return [
...
function requiredToolCallId(context: SessionTaskToolExecutionContext | undefined): string {
...
function notifyChanged(authority: BoundSessionTaskAuthority, revision: number): void {
  try {
    ...
    authority.onObserverError?.({ sessionId: authority.sessionId, revision, error });
  }
}
