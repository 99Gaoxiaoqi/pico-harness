...
export interface ConfiguredSubagentToolExecutionContext {
  readonly signal?: AbortSignal;
}
...
export async function configuredSubagentList(
  ...
  const legacy = SUBAGENT_CAPABILITIES.map((definition) => {
    ...
    return {
      ...
      ...(reason ? { status: "unavailable" as const, reason } : { status: "available" as const }),
    };
...
export class ConfiguredAgentListTool {
  ...
  async execute(args: string, context?: ConfiguredSubagentToolExecutionContext): Promise<string> {
    context?.signal?.throwIfAborted();
    return configuredSubagentList(this.options, args);
  }
}

export class ConfiguredAgentSpawnTool {
  ...
  private async continueChild(
    ...
    context?: ConfiguredSubagentToolExecutionContext,
  ): Promise<string> {
