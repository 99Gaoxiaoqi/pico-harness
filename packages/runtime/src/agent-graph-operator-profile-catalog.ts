import type { RuntimeSubagentPresetContract, SubagentProfile } from "@pico/core";
import {
...
export interface MutableAgentGraphOperatorProfileCatalog extends AgentGraphOperatorProfileCatalog {
  ...
  replaceProfiles(profiles: readonly AgentGraphCatalogProfile[]): void;
}

export function createCatalogAgentGraphOperatorProfileCatalog(
  profiles: readonly AgentGraphCatalogProfile[],
  ...
  const replaceProfiles = (next: readonly AgentGraphCatalogProfile[]) => {
...
export function assertValidAgentGraphOperatorProfileSnapshot(
  ...
  const unsigned: UnsignedProfileSnapshot = {
    ...
    ...(value["subagentPreset"] === undefined
      ? {}
      : { subagentPreset: value["subagentPreset"] as RuntimeSubagentPresetContract }),
    tools,
    permissionPolicy: { mode: "ask", allowSessionGrants: false },
    systemPrompt: {
