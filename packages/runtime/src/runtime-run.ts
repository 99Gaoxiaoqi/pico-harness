...
import { SessionForkRuntimeConflictError } from "@pico/core/session-fork-contract";
import {
  assertIssuedEngineRuntimeCapability,
  type EngineRuntimeCapability,
  type EngineRuntimeWriteGuard,
} from "./runtime-capability.js";
import type { RuntimeLastCompactionCheckpoint as LastCompactionCheckpoint } from "./runtime-port-contract.js";
import {
  COMPACTION_SUMMARY_CLOSE_TAG,
  COMPACTION_SUMMARY_OPEN_TAG,
  type ToolResultEnvelopeInput,
} from "@pico/core";
import { computeCheckpointSourceDigest } from "./runtime-compaction-checkpoint.js";
import {
...
import { normalizeSessionRuntimeStatePatch, type SessionRuntimeStateWritePatch } from "@pico/core";
import {
...
import { buildToolArgumentAudit } from "@pico/core/tool-argument-audit";
import { buildRuntimeToolResultInput } from "./tool-result-builder.js";
import { getRuntimeRunDiagnostics } from "./runtime-run-diagnostics.js";
import {
...
import { admitEventLogNewWork } from "@pico/storage/sqlite/event-log-retention-store";

type DurableRuntimeTranscriptEventRecordedEvent =
  RuntimeTranscriptEventRecordedEvent<DurableTranscriptEvent>;
...
export class RuntimeRun {
  ...
  private readonly runStartedEventId?: string | undefined;
  private readonly terminalEventId?: string | undefined;
  private readonly writeGuard: RuntimeEventWriteGuard;
  private readonly parentRefs?:
    | Pick<RuntimeEventRefs, "parentRunId" | "parentToolCallId">
    | undefined;
  private readonly presentation?: RuntimeRunStartedEvent["data"]["presentation"] | undefined;
  ...
  private pendingMessageCommitBatch?: PendingMessageCommitBatch | undefined;
  ...
  static async repairSessionProjection(
    session: RuntimeProjectionSession,
  ...
  claimsSession(session: RuntimeProjectionSession): boolean {
  ...
  private async recordImportedTranscriptEvent(
    ...
    const event: DurableRuntimeTranscriptEventRecordedEvent = {
  ...
  async recordTranscriptToolResults(
    inputs: readonly ToolResultEnvelopeInput[],
  ...
  registerToolResult(input: ToolResultEnvelopeInput): Message {
    return this.registerToolResultInternal(input, false);
  }

  registerUndispatchedToolResult(input: ToolResultEnvelopeInput): Message {
    return this.registerToolResultInternal(input, true);
  }

  registerProtocolClosureToolResult(input: ToolResultEnvelopeInput): Message {
  ...
  async recordApprovalRequested(
    ...
    this.assertOpen();
    const refs = this.refs({ toolCallId });
  ...
  async recordModelCallStarted(options: RuntimeModelCallStartedOptions): Promise<void> {
    this.assertOpen();
    const refs = this.refs({ providerCallId: options.providerCallId });
    await this.append({
  ...
  async recordModelCallSettled(options: RuntimeModelCallSettledOptions): Promise<void> {
    ...
    await this.append({
      ...
      data: {
        ...
        ...(options.costCNY !== undefined ? { costCNY: options.costCNY } : {}),
        ...(options.costStatus !== undefined ? { costStatus: options.costStatus } : {}),
  ...
  private assertSession(session: RuntimeProjectionSession): void {
...
async function writeWithRuntimeEventGuard<Result>(
  ...
  for (let attempt = 0; attempt < GUARD_RETRY_LIMIT; attempt += 1) {
    ...
    try {
      ...
      await waitForDelay(50 * (attempt + 1));
      continue;
    }
    let result: Result;
    try {
...
function buildInterruptedTranscriptToolStartEvents(options: {
  ...
  readonly at: string;
}): DurableRuntimeTranscriptEventRecordedEvent[] {
  ...
  const activeToolCallIds = new Set(
    inspectDurableTranscriptEvents(transcriptEntries.map(({ event }) => event)).activeToolCallIds,
  );
  ...
  const starts: DurableRuntimeTranscriptEventRecordedEvent[] = [];
...
function canonicalizeRuntimeToolResultInput(
  ...
  try {
    ...
    return JSON.parse(encoded) as ToolResultEnvelopeInput;
  } catch (error) {
...
function normalizeForkSeedEntries(
  ...
  inspectDurableTranscriptEvents(transcriptEvents);
  return normalized;
}
