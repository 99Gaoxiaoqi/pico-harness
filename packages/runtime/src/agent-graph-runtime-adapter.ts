...
import type { RuntimeRunStartedEvent, RuntimeRunTerminalEvent } from "@pico/core";
...
/** The subset of Engine Session options relevant to Graph lookup and pinning. */
export interface AgentGraphSessionOptions {
  readonly picoHome?: string;
  readonly runtimeStorageRoot?: string;
}

export interface AgentGraphManagedSession {
  readonly id: string;
  readonly workDir: string;
}

export interface AgentGraphSessionLease {
  readonly session: AgentGraphManagedSession;
  release(): void;
}

/** Engine-owned session lifecycle injected into the reusable Graph Runtime adapter. */
export interface AgentGraphSessionManagerPort {
  get(
    id: string,
    workDir?: string,
    options?: { readonly picoHome?: string; readonly runtimeStorageRoot?: string },
  ): AgentGraphManagedSession | undefined;
  getOrCreatePinned(
    id: string,
    workDir: string,
    options?: AgentGraphSessionOptions,
  ): Promise<AgentGraphSessionLease>;
}
...
export type AgentGraphExactRunInspection =
  ...
  | Readonly<{
      status: "terminal";
      startEvent: RuntimeRunStartedEvent;
      terminalEvent: RuntimeRunTerminalEvent;
    }>
  | Readonly<{
...
export interface AgentGraphExactRunPort {
  readRunEvents(
    sessionId: string,
    runId: string,
  ): Promise<readonly AgentGraphRuntimeEventEnvelope[]>;
...
export interface EnsureAgentGraphOperatorProvisionInput {
  ...
  readonly workDir: string;
  readonly sessionOptions?: AgentGraphSessionOptions;
}
...
export interface AgentGraphRuntimeAdapterOptions {
  readonly sessionManager: AgentGraphSessionManagerPort;
...
export class AgentGraphRuntimeAdapter implements AgentOutputCommitPort {
  ...
  async ensureOperatorProvision(
    ...
    const existing = this.options.sessionManager.get(
      ...
      input.workDir,
      {
        ...(input.sessionOptions?.picoHome === undefined
          ? {}
          : { picoHome: input.sessionOptions.picoHome }),
        ...(input.sessionOptions?.runtimeStorageRoot === undefined
          ? {}
