...
import type { CredentialRef } from "@pico/core/provider-identity";
import {
  RuntimeConflictError,
  SqliteRuntimeControlStore,
} from "@pico/storage/sqlite/sqlite-runtime-control-store";
import { generateRuntimeId } from "@pico/storage/runtime-control-store-contracts";
import {
  type CronJobRecord,
  type CronRunRecord,
...
export interface CronServiceOptions {
  /** Host-resolved canonical workspace state root holding pico.sqlite. */
  storageRoot: string;
  now?: () => number;
  ownerId?: string;
...
export class CronService {
  ...
  private readonly policyGuard?: CronPolicyGuard | undefined;
