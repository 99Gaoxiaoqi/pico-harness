...
export function summarizeCacheEffectiveness(
  records: readonly ProviderCallCacheRecord[],
): CacheEffectiveness {
...
function countModelSwitches(
  ...
  const sequences = new Map<string, ProviderCallCacheRecord[]>();
  for (const record of records) {
...
function operationalAlerts(
  ...
  const routes = new Map<string, ProviderCallCacheRecord[]>();
  for (const record of records) {
...
function reportedFields(record: ProviderCallCacheRecord): Set<string> {
...
function cacheDiagnosticClassification(
  record: ProviderCallCacheRecord,
...
function cacheMinimumTokens(
  record: Pick<ProviderCallCacheRecord, "provider" | "model">,
): number | undefined {
