...
/** Runtime 工具只需要取消信号和一次工具调用身份；具体 Host 可扩展完整上下文。 */
export interface AgentOutputExecutionContext {
...
/**
 * Graph Operator 的正式输出工具。
 *
 * Runtime 拥有输入、身份、幂等和回执校验；实际提交由 Host 注入的端口处理，
 * 因而不依赖图存储或具体执行入口。
 */
export class AgentOutputTool<TContext extends AgentOutputExecutionContext> {
  ...
  async execute(args: string, execution?: TContext): Promise<string> {
...
/** 创建不耦合 Graph 存储与具体入口的 Operator 工具。 */
export function createAgentOutputTool<TContext extends AgentOutputExecutionContext>(
  options: CreateAgentOutputToolOptions,
): AgentOutputTool<TContext> {
