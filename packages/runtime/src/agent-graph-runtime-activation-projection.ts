...
export interface ProjectAgentGraphRuntimeActivationInput {
  ...
  readonly events: readonly AgentGraphRuntimeEventEnvelope[];
...
function projectDurableActivation(
  ...
  events: readonly AgentGraphRuntimeEventEnvelope[],
): AgentGraphActivationRuntimeProjection {
  ...
  for (const event of events) {
    if (event.kind === "approval.requested") {
      unsettledApprovals.add((event as RuntimeApprovalRequestedEvent).data.approvalId);
    }
    if (event.kind === "approval.settled") {
      unsettledApprovals.delete((event as RuntimeApprovalSettledEvent).data.approvalId);
    }
  }
  return {
    ...
    status: terminal
      ? (terminal as RuntimeRunTerminalEvent).data.status === "completed"
        ? "completed"
        : (terminal as RuntimeRunTerminalEvent).data.status
...
function hasNonAttachableRuntimeFacts(
  ...
  return events.some((event) => {
    ...
    if (
      ...
      (event as RuntimeMessageCommittedEvent).data.message.role !== "user"
    ) {
      throw new AgentGraphRuntimeIntegrityError(
...
function assertEventBelongsToClaim(
  event: AgentGraphRuntimeEventEnvelope,
  claim: AgentGraphActivationClaimRecord,
): void {
  if (
