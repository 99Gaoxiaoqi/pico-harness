...
import type { EngineRuntimePort, EngineSession as Session } from "./agent-engine-ports.js";
...
export interface SubagentExecutionRuntime extends SubagentResponseRuntime {
  ...
  requestedModelRoute?: string | undefined;
  resolvedModelRoute?: string | undefined;
...
export interface SubagentRunnerOptions {
  ...
  readonly usageSession?: Session | undefined;
  readonly runtimePort?: EngineRuntimePort | undefined;
...
export class SubagentRunner {
  ...
  constructor(private readonly options: SubagentRunnerOptions) {
    this.diagnostics = options.diagnostics ?? SILENT_ENGINE_DIAGNOSTICS;
  }
  ...
  async run(
    ...
    const skillIndex = canViewSkills
      ? await this.options.skillLoaderFactory(runtimeWorkspaceRoot).loadAll()
      : "";
    signal?.throwIfAborted();
    ...
    for (;;) {
      ...
      const scheduler = new ToolScheduler<{
        ...
        readonly input?: ToolResultEnvelopeInput | undefined;
              ...
              beforeDispatch: async (finalCall: ToolCall) => {
                await runtimeRun?.recordToolStarted(
                  ...
                  finalCall.arguments,
                  {
                    ...(step === undefined ? {} : { step }),
                    origin: "model",
                    ...(readOnlyRegistry.getRecoveryPolicy
