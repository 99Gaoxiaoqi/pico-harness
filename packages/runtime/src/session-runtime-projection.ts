...
export interface SequencedRuntimeEvent {
  ...
  readonly event: RuntimeEvent<DurableTranscriptEvent>;
}
...
/** raw model-message 投影消费的 kind 集。 */
...
/**
 * Pure Runtime projections over the durable Session ledger. Storage owns the
 * concrete store; this module owns no host, database, or execution-loop state.
 */
export function projectRuntimeSessionMessages(
  events: readonly RuntimeEvent<DurableTranscriptEvent>[],
): Message[] {
...
export function projectRuntimeSessionUsage(
  events: readonly RuntimeEvent<DurableTranscriptEvent>[],
...
function projectModelHistoryEvents(
  events: readonly RuntimeEvent<DurableTranscriptEvent>[],
): RuntimeModelHistoryEvent[] {
  return selectRuntimeEvents(events, runtimeEventHasModelHistoryEntry).map(({ event }) => event);
}

/** Select matching events while retaining their source index and rejecting duplicate IDs. */
function selectRuntimeEvents<Event extends RuntimeEvent<DurableTranscriptEvent>>(
  events: readonly RuntimeEvent<DurableTranscriptEvent>[],
  select: (event: RuntimeEvent<DurableTranscriptEvent>) => event is Event,
...
function isRuntimeToolResult(
  event: RuntimeEvent<DurableTranscriptEvent>,
): event is RuntimeToolResultRecordedEvent {
