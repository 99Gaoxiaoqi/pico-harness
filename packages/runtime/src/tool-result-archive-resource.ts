...
interface ToolResultArchiveResourceIdentity {
  artifactId: string;
...
export async function readToolResultArchiveResource(
  reader: BoundToolResultArchiveReader,
  request: ToolResultArchiveResourceRequest,
  abortSignal?: AbortSignal,
): Promise<unknown> {
  const identity = parseToolResultArchiveRef(request.ref);
  if (!identity) {
    return archiveFailure(request.ref, "invalid_ref");
  }
  if (identity.sizeBytes > TOOL_RESULT_ARCHIVE_MAX_BYTES) {
    return archiveFailure(request.ref, "too_large", {
      originalBytes: identity.sizeBytes,
      maxBytes: TOOL_RESULT_ARCHIVE_MAX_BYTES,
    });
  }
  if (abortSignal?.aborted) throw new Error("archive_read aborted");

  let serializedResult: string;
  try {
    serializedResult = await reader.readRaw(request.ref);
  } catch {
    return archiveFailure(request.ref, "read_failed");
  }
  if (abortSignal?.aborted) throw new Error("archive_read aborted");
  ...
  const parsed = deserializeArchive(serializedResult);

  if (operation === "search") {
    const text = resolveArchiveText(parsed, serializedResult);
  ...
  return inspectArchive(
    request.ref,
    { artifactId: identity.eventId, originalBytes: identity.sizeBytes },
    parsed,
    serializedResult,
  );
}
...
export const TOOL_RESULT_ARCHIVE_READ_INSTRUCTIONS =
  'This result is archived but still readable. Call archive_read with the provided ref and operation "inspect"; use operation "query" with itemId for one structured item, or operation "read" with offset/limit for a bounded page. Do not use Glob to find the archive.';
...
function inspectArchive(
  ...
  const readHint =
    'Call archive_read with operation "read" (unit "char" or "line"), offset, and limit for bounded pages, or operation "search" with a pattern to locate text.';
  ...
  if (isRecord(value)) {
    ...
    if (items) {
      const queryHint =
        'Call archive_read with operation "query" and one of the listed itemId values.';
