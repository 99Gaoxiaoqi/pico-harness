...
export class AtomicMemoryExtractionEngine {
  ...
  private range(
    ...
    const { sourceTools: _sourceTools, ...snapshotWithoutSourceTools } = snapshot;
    return {
      snapshot:
        snapshot.trigger === "compaction"
          ? {
              ...snapshotWithoutSourceTools,
...
function historicalSnapshot(
  ...
  const {
    sourceTools,
    compactionCheckpointId: _currentCompactionCheckpointId,
    ...baseSnapshot
  } = current;
  return {
    ...baseSnapshot,
    ...
    ...(trigger !== "compaction" && sourceTools !== undefined ? { sourceTools } : {}),
    ...(compactionCheckpointId !== undefined ? { compactionCheckpointId } : {}),
  };
}
...
function freezeSnapshot(input: MemoryExtractionSnapshot): MemoryExtractionSnapshot | undefined {
  ...
  const frozenCheckpoints: readonly MemoryCheckpointBoundary[] = structuredClone(
    input.checkpoints ?? [],
  );
  const snapshot: MemoryExtractionSnapshot = {
    ...
    ...(input.trigger !== "compaction" && sourceTools !== undefined
...
function requestBudget(
  snapshot: MemoryExtractionSnapshot,
  stage: MemoryModelRequest["stage"],
): MemoryRequestBudgetInput {
  return {
    ...(snapshot.sourceMessages !== undefined ? { sourceMessages: snapshot.sourceMessages } : {}),
    ...(snapshot.trigger !== "compaction" &&
    stage === "proposal" &&
    snapshot.sourceTools !== undefined
      ? { sourceTools: snapshot.sourceTools }
      : {}),
    ...(snapshot.contextWindowTokens !== undefined
      ? { contextWindowTokens: snapshot.contextWindowTokens }
      : {}),
    ...(snapshot.reservedOutputTokens !== undefined
