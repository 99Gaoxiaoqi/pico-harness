...
import { type Message, type ToolCall, type ToolDefinition, type ToolResult } from "@pico/core";
import { ToolCommitBoundaryError, type ToolExecutionStep } from "./runtime-tool-execution.js";
import {
  type EngineToolRegistry as Registry,
  type EngineToolFileSideEffects as ToolFileSideEffects,
  type EngineSession as Session,
...
function isBackgroundBashCall(call: ToolCall): boolean {
  ...
  try {
    const input = JSON.parse(call.arguments) as { background?: unknown | undefined };
...
export interface AgentEngineOptions {
  readonly host: EngineHostServices;
  readonly diagnostics?: EngineDiagnostics | undefined;
  ...
  systemPromptFactory?: (() => Promise<string>) | undefined;
  /**
   ...
   */
  recovery?: RecoveryManager | undefined;
  /**
   * 死循环探测器:连续同参数失败时注入 [SYSTEM REMINDER] 强行打断 (第 15 讲)。
   * 未提供则默认创建一个,阈值 3 次同参数失败触发干预。
   */
  reminderInjector?: ReminderInjector | undefined;
   ...
   */
  goalManager?: GoalManager | undefined;
  /**
   ...
   * 未提供则行为不变(全量工具喂给 LLM)。
   */
  toolDisclosure?: ToolDisclosure | undefined;
  /** 可选的轮次日志回调,便于第 19 讲 Tracing 接入 */
  onTurn?: ((info: { turn: number; message: Message }) => void) | undefined;
  /**
   * 可信宿主登记的精确敏感值。所有工具结果在进入 recovery、Provider transcript、
   * Runtime 持久化、Hook 与 trace 前按完整字符串替换；工具参数不能扩充此列表。
   */
  toolResultRedactionSecrets?: readonly string[] | undefined;
  ...
  waitAtSafeBoundary?: (() => Promise<void>) | undefined;
  /**
  ...
  postToolResultHook?: ((call: ToolCall, result: ToolResultEnvelope) => Promise<void>) | undefined;
...
export class AgentEngine {
  private readonly host: EngineHostServices;
  private readonly diagnostics: EngineDiagnostics;
  private provider: LLMProvider;
  private readonly registry: Registry;
  private readonly workDir: string;
  private readonly systemPrompt: string;
  private readonly systemPromptFactory?: (() => Promise<string>) | undefined;
  private readonly promptLayersFactory?: AgentEngineOptions["promptLayersFactory"] | undefined;
   ...
   */
  private steerQueue?: SteerQueue | undefined;
  private readonly waitAtSafeBoundary?: (() => Promise<void>) | undefined;
  /** 非工具停止后续接回调(ROADMAP 3.7):host 可决定让 Agent 接着跑 */
  private readonly shouldContinueAfterStop?:
    | AgentEngineOptions["shouldContinueAfterStop"]
    | undefined;
  /** 凭证轮换回调(4.2):429 时切换 key 重建 provider;无多 key 时为 undefined */
  private readonly rebuildProvider: AgentEngineOptions["rebuildProvider"];
  private readonly hookService?: HookService | undefined;
  ...
  constructor(opts: AgentEngineOptions) {
    this.host = opts.host;
    this.diagnostics = opts.diagnostics ?? SILENT_ENGINE_DIAGNOSTICS;
  ...
  private async buildPromptLayers(
    ...
    if (this.isPlanning()) {
      return this.host.buildPlanPrompt(signal);
    }
  ...
  async run(
    ...
    const capability = this.host.sessionCapability(session);
  ...
  private async executeSession(
    ...
    const fileHistory = this.host.createFileHistoryScope(session, this.registry);
    ...
    try {
      for (;;) {
        ...
        this.registry.setPreWriteHook?.((toolName, args) =>
          fileHistory.trackToolWrite(toolName, args),
        );
        ...
        try {
          ...
          try {
            try {
              ...
              if (hasFileEffects) {
                await fileHistory.beginJournal(signal);
                if (toolCalls.some((call) => isBackgroundBashCall(call))) {
                  fileHistory.addJournalWarning(
      ...
      await this.onRunInterrupted?.(
        ...
        this.diagnostics.warn({ hookError: String(hookError) }, "[Engine] 运行中断收口失败"),
      );
      throw error;
    } finally {
      ...
      rootSpan?.end();
      if (rootSpan) {
        const tracePath = this.host.exportTrace(rootSpan, session);
        this.diagnostics.info({ tracePath }, `[Tracing] 执行回放链路已保存: ${tracePath}`);
      }
    }
  ...
  private async runOneTool(
    ...
    try {
        ...
        result = await this.registry.execute(toolCall, {
          ...
          beforeDispatch: async (finalCall) => {
            await runtimeRun?.recordToolStarted(finalCall.id, finalCall.name, finalCall.arguments, {
              ...(step === undefined ? {} : { step }),
              origin: "model",
              ...(this.registry.getRecoveryPolicy
      ...
      const builtResult = runtimeRun
        ...
        : buildEphemeralToolResult(toolCall, result, finalOutput, runtimeStatus, this.diagnostics);
