import { type RuntimeEffectiveConfig, type RuntimeUserConfig } from "@pico/protocol";
...
/** Confirmation remains scoped to the caller's command session. No secret is retained here. */
...
export interface AutomationCommandRegistryDeps {
  readonly runtime: RpcCommandRuntime;
  readonly workspacePath: string;
  readonly credentialEnv?: Readonly<Record<string, string | undefined>>;
  readonly allowedTools: readonly string[];
  readonly automationCredentialProposals: {
    issue(binding: AutomationCredentialBinding): {
      readonly proposalId: string;
      readonly expiresAt: number;
    };
    consume(
      proposalId: string,
      binding: AutomationCredentialBinding,
    ): {
      readonly status: "accepted" | "missing" | "expired" | "changed";
    };
  };
  readonly resolveCredentialTarget: (input: {
    readonly route: ClientAutomationAuthority["route"];
    readonly userProvider?: Pick<
      RuntimeUserConfig["providers"][number],
      "protocol" | "modelProtocols" | "baseURL" | "apiKeyEnv" | "auth" | "models" | "discoverModels"
    >;
    readonly configSource?: "user" | "environment" | "session" | "cli";
  }) => { readonly ref: string };
}

export function createAutomationCommands(deps: AutomationCommandRegistryDeps) {
  const { runtime, workspacePath } = deps;
  const { session, needSession } = sessionAccess(runtime);
  const automationCredentialProposals = deps.automationCredentialProposals;
        ...
        try {
          ...
          if (operation === "credential") {
            ...
            const authority = await resolveClientAutomationAuthority({
              resolveCredentialTarget: deps.resolveCredentialTarget,
              runtime,
              workspacePath,
...
async function resolveClientAutomationAuthority(input: {
  readonly runtime: RpcCommandRuntime;
  readonly resolveCredentialTarget: AutomationCommandRegistryDeps["resolveCredentialTarget"];
  readonly workspacePath: string;
  readonly requestedRouteId?: string | undefined;
  ...
  const target = input.resolveCredentialTarget({
    route,
