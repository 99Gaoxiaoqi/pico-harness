...
} from "@pico/cli/command-availability";
import { isPartialSlashCommandName } from "@pico/core/slash-parser";
import type {
  CommandListOptions,
  ...
  SlashCommandKind,
  SlashCommandSource,
} from "@pico/cli/command-contracts";
    ...
    if (normalized.length === 0) {
      return this.list(
        options.availabilityState === undefined
          ? {}
          : { availabilityState: options.availabilityState },
      ).map((command) => suggestionFromCommand(command));
    }
