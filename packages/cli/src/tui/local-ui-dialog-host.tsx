...
import type { LocalUiCommandAction } from "@pico/cli/command-contracts";
...
export interface LocalUiDialogHostContext {
  commands?: readonly HelpPanelCommand[] | undefined;
  models?: readonly ModelOption[] | undefined;
  currentModelId?: string | undefined;
  /** 模型选择确认（3-D 客户端接线；in-process 选择器由 repl 自管）。 */
  onModelConfirm?: ((model: ModelOption) => void) | undefined;
  sessions?: readonly SessionBrowserSession[] | undefined;
  /** 会话选择确认 → 宿主派发（mode 区分 /resume 与 /fork）。 */
  onSessionConfirm?:
    | ((session: SessionBrowserSession, mode: "resume" | "fork") => void)
    | undefined;
  rewindSessionId?: string | undefined;
  rewindSnapshots?: readonly FileHistorySnapshotSummary[] | undefined;
  /** rewind preview 数据源（3-D 客户端：rewind.preview RPC 的宿主桥）。 */
  rewindGetDiffStat?: ((messageId: string) => Promise<FileHistoryDiffStat>) | undefined;
  /** rewind 应用回调（rewind.apply RPC；viewOnly 查看型由宿主拦截提示）。 */
  rewindOnApply?:
    | ((snapshot: FileHistorySnapshotSummary, mode: RewindMode) => Promise<void>)
    | undefined;
  /** 查看型（/changes）：confirm 动作不执行回滚。 */
  rewindViewOnly?: boolean | undefined;
  /** 预选 checkpoint（/changes <message-id>）。 */
  rewindSelectedMessageId?: string | undefined;
  /** /changes 单文件恢复（tier2 收口）：checkpoint id + 模型数据源 + 动作回调。 */
  changesMessageId?: string | undefined;
  changesLoadModel?: ((messageId: string) => Promise<ChangesPanelModel>) | undefined;
  changesOnRestoreFile?: ((action: ChangesRestoreFileAction) => void | Promise<void>) | undefined;
  changesOnJumpToRewind?: ((action: ChangesJumpToRewindAction) => void | Promise<void>) | undefined;
  onClose?: ((id: string) => void) | undefined;
  maxHelpItems?: number | undefined;
}
