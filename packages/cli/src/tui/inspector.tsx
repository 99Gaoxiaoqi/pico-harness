import { optionalTextColor } from "./text-style.js";
import React, { useCallback, useEffect, useRef, useState } from "react";
import { Box, Text, useInput } from "ink";
import type { TranscriptToolCallProjection as TuiToolCallProjection } from "@pico/pico-host/transcript-event-store";
...
export interface InspectorPage {
  ...
  /** 宿主可直接交给剪贴板动作。 */
  copyText: string;
  availability?: ("complete" | "unavailable") | undefined;
}
...
export interface InspectorDialogContentProps {
  ...
  loadPage?: typeof readInspectorPage | undefined;
}
    ...
    <Box flexDirection="column">
      {compact ? (
        <>
          ...
          <Text {...optionalTextColor(error ? "red" : undefined)} wrap="truncate">
            {truncateTerminalText(
              error ??
                (page
                  ? (page.content.split("\n")[lineOffset] ?? "")
