...
import type { AskUserOption } from "@pico/pico-host/ask-user-tool";
...
export interface ClientSessionRuntimeOptions {
  ...
  readonly onApproval?: ((notice: ApprovalNotice) => void) | undefined;
  /** 审批被解析（含对端/超时解析）——宿主清理残留对话框。 */
  readonly onApprovalResolved?: ((approvalId: string) => void) | undefined;
  /** 可操作的持久化 PlanControl 投影——不经工具审批链。 */
  readonly onPlanControl?: ((notice: PlanControlNotice) => void) | undefined;
  /** PlanControl 变为不可操作或被新 epoch 替换。 */
  readonly onPlanControlResolved?: ((controlId: string) => void) | undefined;
  /** ask-user 问题到达（已从 wire prompt 映射为 AskUserRequest 形状；宿主开对话框）。 */
  readonly onPrompt?: ((request: ClientPromptRequest) => void) | undefined;
  /** 问题被解析（含对端取消/answered）——宿主清理残留对话框。 */
  readonly onPromptResolved?: ((promptId: string) => void) | undefined;
  readonly onRunStateChanged?: ((running: boolean) => void) | undefined;
  ...
  readonly thinkingOverride?: string | undefined;
  /** --graph / --swarm 启动覆盖，在首次发送或恢复会话时生效。 */
  readonly orchestrationModeOverride?: ("graph" | "swarm") | undefined;
}
...
export class ClientSessionRuntime {
  ...
  async sendText(
    ...
    execution?: { orchestrationMode?: ("graph" | "swarm") | undefined },
  ): Promise<boolean> {
  ...
  private hydrateSerial(
    options: { readonly propagateError?: boolean | undefined } = {},
  ): Promise<void> {
