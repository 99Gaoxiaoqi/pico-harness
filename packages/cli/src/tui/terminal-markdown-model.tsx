...
interface InlineStyle {
  ...
  readonly color?: ("cyan" | "yellow" | "gray") | undefined;
}
...
function tokenChildren(token: Token): Token[] | undefined {
  const children = (token as { tokens?: unknown | undefined }).tokens;
  return Array.isArray(children) ? (children as Token[]) : undefined;
}

function tokenText(token: Token): string {
  const text = (token as { text?: unknown | undefined }).text;
