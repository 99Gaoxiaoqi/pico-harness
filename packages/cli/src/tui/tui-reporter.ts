...
import type {
  ...
  SubagentTraceEvent,
} from "@pico/core/reporter-contract";
...
import { summarizeTranscriptToolResult } from "@pico/pico-host/transcript-tool-result-hydration";
import {
  defaultTranscriptDurabilityPolicy,
  isDurableTranscriptEvent,
  type DurableTranscriptSink,
  type TranscriptDurabilityPolicy,
} from "@pico/pico-host/transcript-durability";
import {
...
export interface TuiReporterOptions {
  /** 水合或回放时可以传入已有事件库。 */
  eventStore?: TranscriptEventStore | undefined;
  /** 消费带稳定 ID 的唯一权威投影。 */
  onProjectionUpdate?: ((projection: TuiProjection) => void) | undefined;
  /** 将语义 transcript 事件串行写入 Session；不持久化逐 token delta。 */
  durableTranscriptSink?: DurableTranscriptSink | undefined;
  durableTranscriptPolicy?: TranscriptDurabilityPolicy | undefined;
  /** 已持久化 transcript 的最大 sequence，用于续写时保持连续。 */
  durableTranscriptSequence?: number | undefined;
}
...
export class TuiReporter implements Reporter {
  ...
  private readonly onProjectionUpdate?: ((projection: TuiProjection) => void) | undefined;
  private readonly durableTranscriptSink?: DurableTranscriptSink | undefined;
  ...
  pushError(
    message: string,
    options: { retryable?: boolean | undefined; action?: string | undefined } = {},
  ): void {
    this.appendEntry({
      kind: "error",
      message,
