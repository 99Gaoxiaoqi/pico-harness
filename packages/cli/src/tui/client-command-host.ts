...
export interface ClientCommandHostDeps {
  ...
  readonly currentModelId?: (() => string | undefined) | undefined;
  ...
  readonly getRewindDiffStat?: ((messageId: string) => Promise<FileHistoryDiffStat>) | undefined;
  /** rewind 应用（rewind.apply RPC 桥；viewOnly 查看型不触发）。 */
  readonly onRewindApply?:
    | ((snapshot: FileHistorySnapshotSummary, mode: RewindMode) => void | Promise<void>)
    | undefined;
  /** /changes 单文件恢复（tier2 收口）：模型数据源（rewind.changes RPC 桥）。
   * 缺省时 /changes 降级为提示（不打开对话框）。 */
  readonly getRewindChanges?: ((messageId: string) => Promise<ChangesPanelModel>) | undefined;
  /** 单文件恢复执行（rewind.restoreFile RPC 桥）。 */
  readonly onRestoreRewindFile?:
    | ((action: ChangesRestoreFileAction) => void | Promise<void>)
    | undefined;
  /** Changes 面板 → 完整回滚跳转（默认派发 /rewind <id>）。 */
  readonly onJumpToRewind?:
    | ((action: ChangesJumpToRewindAction) => void | Promise<void>)
    | undefined;
}
...
export function handleClientLocalCommand(
  ...
  if (result.ui) {
    ...
    if (result.ui.kind === "open-selector" && result.ui.selector === "model") {
      const routes = (
        result.data as { modelRoutes?: { id: string; name: string }[] | undefined } | undefined
      )?.modelRoutes;
    ...
    if (result.ui.kind === "open-selector" && result.ui.selector === "rewind") {
      const data = result.data as
        | {
            ...
            snapshots?: FileHistorySnapshotSummary[] | undefined;
    ...
    if (result.ui.kind === "open-selector" && result.ui.selector === "changes") {
      const data = result.data as
        | { sessionId?: string | undefined; checkpointId?: string | undefined }
        | undefined;
  ...
  switch (result.action) {
    ...
    case "resume": {
      const data = result.data as
        | { mode?: string | undefined; sessionId?: string | undefined }
        | undefined;
