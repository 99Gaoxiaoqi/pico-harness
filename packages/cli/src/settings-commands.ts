...
import type { ClientCommandRuntime } from "./resources-commands.js";
import {
  rpcCommand,
  staticCompleter,
  sessionAccess,
...
/** Settings commands consume only session settings and input capabilities. */
export interface SettingsCommandRuntime extends CommandSessionRuntime {
  readonly preSessionSettings: Readonly<
    Pick<RuntimeUserDefaults, "collaborationMode" | "permissionMode" | "orchestrationMode">
  >;
  request<Method extends "session.settings.get" | "session.settings.update">(
    method: Method,
    params: RuntimeParams<Method>,
  ): Promise<RuntimeResult<Method>>;
  sendInput: ClientCommandRuntime["sendInput"];
...
export interface SettingsCommandRegistryDeps {
  readonly runtime: SettingsCommandRuntime;
  readonly workspacePath: string;
}

export function createSettingsCommands(deps: SettingsCommandRegistryDeps) {
