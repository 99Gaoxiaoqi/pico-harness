...
import { RUNTIME_FORK_BOOTSTRAP_RUN_PREFIX } from "@pico/core";
...
export interface UsageRuntimeEventReader {
  listSessionCatalogEntries(): Promise<
    readonly {
      readonly summary: {
        readonly id: string;
  ...
  >;
  readUsageToolMetadata(sessionId?: string): readonly UsageRuntimeToolMetadata[];
  close(): void;
}

export interface UsageRuntimeToolMetadata {
...
export interface UsageDashboardInput {
  readonly sources: readonly {
    ...
    storageRoot: string;
    calls: readonly UsageProviderCallRecord[];
  }[];
  ...
  /** Storage-owned Runtime ledger reader supplied by the outer host composition. */
  readonly createRuntimeEventReader: (storageRoot: string) => UsageRuntimeEventReader;
}
...
export async function buildUsageDashboard(
  ...
  for (const source of input.sources) {
    ...
    let store: UsageRuntimeEventReader | undefined;
    try {
      store = input.createRuntimeEventReader(source.storageRoot);
      ...
      for (const event of store.readUsageToolMetadata(input.sessionId)) {
        if (event.runId.startsWith(RUNTIME_FORK_BOOTSTRAP_RUN_PREFIX)) continue;
...
function providerLabel(call: UsageProviderCallRecord): string {
  if (call.route) {
    try {
