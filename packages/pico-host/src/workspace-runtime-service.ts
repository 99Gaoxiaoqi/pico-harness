...
import { readEventLogStorageStatus } from "@pico/storage/sqlite/event-log-retention-store";
import {
...
import { trimRuntimeNotificationToFit } from "./runtime-notification-transport.js";
import { INTERRUPTED_DAEMON_RUN_ERROR as interruptedDaemonRunError } from "./workspace-run-lifecycle.js";
import { WorkspaceRegistrationStore } from "./workspace-registration.js";
import {
  runWorkspaceBlobGcOnce as runStorageWorkspaceBlobGcOnce,
...
export class WorkspaceRuntimeService implements DisposableLocalRuntimeService {
  ...
  private readonly blobGcTimers = new Map<
    string,
    { readonly wakeAt: number; readonly timer: ScheduledDeadline }
  >();

  constructor(private readonly options: WorkspaceRuntimeServiceOptions) {
    this.picoHome = resolvePicoHome(options.env ? { env: options.env } : {});
  ...
  closeRuntimes(): Promise<void> {
    ...
    delete this.registrationChanged;
  ...
  private clearAllBlobGcTimers(): void {
    for (const { timer } of this.blobGcTimers.values()) timer.cancel();
  ...
  private withDeferredNotifications<Result>(execute: () => Result): Result {
    ...
    try {
      ...
      if (!parent) delete this.deferredNotifications;
    }
...
function projectWorkspaceRuntimeNotification(
  event: import("./workspace-task-runtime.js").WorkspaceRuntimeEvent,
...
/** Bind the Storage GC to the same Host workspace layout as the Runtime stores. */
async function runWorkspaceBlobGcOnce(options: {
  workDir: string;
  picoHome: string;
}): Promise<WorkspaceBlobGcResult> {
  const paths = resolvePicoPaths(options.workDir, { picoHome: options.picoHome });
  return runStorageWorkspaceBlobGcOnce({
    paths: {
