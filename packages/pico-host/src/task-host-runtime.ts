...
import { buildSafeGitEnvironment } from "@pico/runtime/git-safety";
...
export interface TaskHostRuntimeLogger {
  error(bindings: object, message: string): void;
  warn(bindings: object, message: string): void;
}

const NOOP_LOGGER: TaskHostRuntimeLogger = {
  error: () => undefined,
  warn: () => undefined,
};
...
export class TaskHostRuntime {
  ...
  private readonly logger: TaskHostRuntimeLogger;
  ...
  private constructor(
    ...
    runnerStopTimeoutMs?: number,
    logger: TaskHostRuntimeLogger = NOOP_LOGGER,
  ) {
    ...
    this.reconcileTimer = setInterval(
      () => {
        try {
          ...
          this.logger.warn({ error: String(error) }, "[runtime-store] 过期任务收口失败");
        }
      },
  ...
  static async create(options: TaskHostRuntimeOptions): Promise<TaskHostRuntime> {
    ...
    const { service } = await JobService.create({
      storageRoot: resolvePicoPaths(repoRoot, {
