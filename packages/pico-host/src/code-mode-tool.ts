...
import type { RuntimeRunPort } from "@pico/runtime/runtime-port-contract";
import {
...
/** The cell needs nested dispatch only, never an Engine Session or its lifecycle. */
export type CodeModeRuntimeRun = Pick<
  RuntimeRunPort<unknown, Registry, ToolExecutionContext, unknown>,
  "executeNestedTool"
>;
...
export interface CodeModeToolOptions {
  ...
  readonly getRuntimeRun?: () => CodeModeRuntimeRun | undefined;
  readonly diagnostics?: RuntimeToolResultLogger;
  ...
  definition(): ToolDefinition {
    return {
      ...
      description: [
        ...
        "Use exec as the preferred path for two or more independent nestable tool calls, repeated calls over a collection, or deterministic filtering, comparison, or aggregation of tool results.",
        "For example, to compare dependencies across known config files or total selected records, read the files in parallel inside exec and return the differences or totals.",
        "Keep intermediate data inside the cell and return only the relevant evidence or summary needed for the next decision.",
        "Use a direct tool call for a single simple operation. Keep steps separate when choosing the next action requires semantic judgment about unexpected observations; ordinary parsing and filtering belong inside exec.",
        "Do not batch approval-sensitive or irreversible actions merely to reduce calls; completed child effects are not automatically rolled back when a later call fails.",
        ...
        "In particular, read_file returns numbered text lines plus status text, not an object with a content field or raw JSON. Extract the file lines and remove their line-number prefixes before parsing JSON file content.",
        "For a complete, untruncated JSON file read, use: const body = output.split('\\n').filter(line => /^\\d+\\t/.test(line)).map(line => line.replace(/^\\d+\\t/, '')).join('\\n'); const data = JSON.parse(body); This discards the unnumbered status/footer lines. If the output is partial or clipped, obtain the missing content before parsing.",
