...
const NOOP_LOGGER: HookExecutorLogger = {
  warn: () => undefined,
  error: () => undefined,
};

let logger: HookExecutorLogger = NOOP_LOGGER;

export function configureHookExecutorLogger(next: HookExecutorLogger): void {
  logger = next;
}
...
function terminateProcessTree(child: ChildProcess): Promise<void> {
  ...
  return waitForDelay(ABORT_KILL_GRACE_MS).then(() => signalProcess("SIGKILL"));
}
