...
export interface LoadHookSnapshotOptions {
  ...
  version?: number;
  logger?: HookConfigLogger;
}

export interface HookConfigLogger {
  warn(context: Readonly<Record<string, unknown>>, message: string): void;
}

const NOOP_LOGGER: HookConfigLogger = { warn: () => undefined };
...
export function defaultHookConfigSources(
  ...
  const resolvedPicoHome = picoHome ?? resolvePicoHome();
  return [
...
export async function loadHookSnapshot(
  ...
  const specs = [
    ...defaultHookConfigSources(workspace, options.picoHome),
    ...
    for (const event of HOOK_EVENTS) {
      for (const group of loaded.config[event] ?? []) {
        for (const handler of group.hooks) {
          ...
          const trust = await executableTrustStatus(
            handler,
            trustStore,
            workspace,
            loaded.source,
...
function normalizeHandler(input: unknown): HookHandler {
  ...
  switch (type) {
    ...
    case "http": {
      ...
      const headers = optionalStringRecord(input.headers, "headers");
      const allowedEnv = optionalStringArray(input.allowedEnv, "allowedEnv");
      return {
        type,
        url,
        ...common,
