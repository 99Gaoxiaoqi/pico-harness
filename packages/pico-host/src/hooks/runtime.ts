...
import type { WorkspaceTrustStore } from "../workspace-trust.js";
import {
  createSandboxPolicy,
...
export interface HookRuntimeLogger extends HookConfigLogger, HookExecutorLogger {
  warn(contextOrMessage: Readonly<Record<string, unknown>> | string, message?: string): void;
}

const NOOP_LOGGER: HookRuntimeLogger = {
  warn: () => undefined,
  error: () => undefined,
};

export interface HookManagementCommandFactoryInput {
  management: HookManagementService;
  hookify: (description: string) => Promise<{ proposal: HookifyProposal; applied: boolean }>;
}

export interface SessionHookRuntimeOptions<Command = never> extends Pick<
...
> {
  ...
  logger?: HookRuntimeLogger;
  commandFactory?: (input: HookManagementCommandFactoryInput) => readonly Command[];
}

export interface SessionHookRuntime<Command = never> {
  service: HookService;
...
export async function createSessionHookRuntime<Command = never>(
  options: SessionHookRuntimeOptions<Command>,
): Promise<SessionHookRuntime<Command>> {
  ...
  let rules = await safeLoadHookifyRules(options.workDir, logger);
  ...
  const commands = options.commandFactory?.({ management, hookify }) ?? [];
...
async function safeLoadHookifyRules(
  workDir: string,
  logger: HookRuntimeLogger,
): Promise<readonly HookifyRule[]> {
  try {
