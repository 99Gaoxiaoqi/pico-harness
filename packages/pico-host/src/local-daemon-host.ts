...
export interface DaemonCronRuntime<RunResult = unknown> {
  recoverInterruptedRuns(reason?: string): readonly RunResult[];
  runNow(cronJobId: string): RunResult;
  start(): void;
  beginClose(): void;
  close(): Promise<void>;
...
export interface DaemonCronRuntimeFactory<RunResult = unknown> {
  create(input: {
    readonly workspacePath: string;
    readonly ownerId: string;
  }): Promise<DaemonCronRuntime<RunResult>>;
}

export interface LocalDaemonHostOptions<RunResult = unknown> {
  service: DisposableLocalRuntimeService;
  cronRuntimeFactory: DaemonCronRuntimeFactory<RunResult>;
  registrationStore?: WorkspaceRegistrationStore;
  ownerId?: string;
  onWorkspaceError?: (workspacePath: string, error: unknown) => void;
  onStartupCleanupFailure?: (cleanupError: unknown, startupError: unknown) => void;
}
 ...
 */
export class LocalDaemonHost<RunResult = unknown> {
  readonly ownerId: string;
  private readonly registrationStore: WorkspaceRegistrationStore;
  private readonly cronRuntimes = new Map<string, DaemonCronRuntime<RunResult>>();
  private readonly cronShutdownRuntimes = new Map<string, DaemonCronRuntime<RunResult>>();
  private readonly cronShutdownFailures = new Map<string, unknown>();
  private state: HostState = "stopped";
  private serviceClosed = false;
  private serviceClosePromise: Promise<void> | undefined;
  private startPromise: Promise<void> | undefined;
  private stopPromise: Promise<void> | undefined;
  private reconcileQueue: Promise<void> = Promise.resolve();

  constructor(private readonly options: LocalDaemonHostOptions<RunResult>) {
  ...
  private async startOnce(): Promise<void> {
    try {
      ...
      try {
        ...
        this.options.onStartupCleanupFailure?.(cleanupError, error);
      } finally {
        this.state = "stopped";
      }
      throw error;
  ...
  async runCronJobNow(workspacePath: string, cronJobId: string): Promise<RunResult> {
  ...
  private trackCronOwnershipRelease(
    ...
    runtime: DaemonCronRuntime<RunResult>,
  ): void {
...
function readCronOwnershipFence<RunResult>(
  runtime: DaemonCronRuntime<RunResult>,
...
export function installLocalDaemonShutdownHandlers<RunResult>(
  host: LocalDaemonHost<RunResult>,
