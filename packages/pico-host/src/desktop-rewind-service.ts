...
import { StorageOperationJournal } from "@pico/storage";
import {
  projectDesktopRewindFingerprints,
  type DesktopCheckpointSession,
  type ReadDesktopCheckpointChanges,
...
export type DesktopRewindMode = RuntimeParams<"rewind.apply">["mode"];

/** Engine-facing session capability required by the Host rewind workflow. */
export interface DesktopRewindSession<
  FileHistory,
  RuntimePort,
> extends DesktopCheckpointSession<FileHistory> {
  getRuntimeStateSnapshot(): { readonly settings?: unknown };
  forkFromCheckpoint(
    checkpointId: string,
    mode: DesktopRewindMode,
...
export interface DesktopRewindLogger {
  warn(fields: Readonly<Record<string, unknown>>, message: string): void;
}

/** Host-owned dependencies; Engine, File History and logging remain injected adapters. */
export interface DesktopRewindServiceOptions<FileHistory, RuntimePort> {
  ...
  readonly withSession: <Result>(
    workspacePath: string,
    sessionId: string,
    operation: (session: DesktopRewindSession<FileHistory, RuntimePort>) => Promise<Result>,
  ) => Promise<Result>;
  ...
  readonly forkRuntimePort: RuntimePort;
  readonly readChanges: ReadDesktopCheckpointChanges<FileHistory>;
  readonly logger?: DesktopRewindLogger;
}

/** Owns rewind idempotency and the non-destructive fork/file transaction lifecycle. */
export class DesktopRewindService<FileHistory, RuntimePort> {
  ...
  constructor(private readonly options: DesktopRewindServiceOptions<FileHistory, RuntimePort>) {}
  ...
  private async applyOnce(
    ...
    const forkedSessionId = await this.options.withSession(
      ...
      async (session) => {
        ...
        const forkJournal =
          mode === "both"
            ? new StorageOperationJournal({
                storageRoot: resolvePicoPaths(canonical, { picoHome: this.options.picoHome })
                  .workspace.root,
              })
            : undefined;
