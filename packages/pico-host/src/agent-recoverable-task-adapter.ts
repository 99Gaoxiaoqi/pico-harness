...
import { RECOVERABLE_TASK_LAUNCH_RECEIPT_SCHEMA_VERSION } from "@pico/core/task-run-contract";
import {
...
import {
  ...
  withFileLock,
  writeJsonAtomicSync,
} from "@pico/storage/local-file-storage";
import {
...
} from "@pico/storage/sqlite/sqlite-workspace-storage";
import { ALL_WORKSPACE_SQLITE_SCOPES } from "@pico/storage/sqlite/workspace-scopes";
  ...
  async installOrConfirm(
    ...
    for (;;) {
      ...
      if (claim.status === "waiting") {
        ...
        await waitForDelay(this.contentionPollMs);
        continue;
      }
