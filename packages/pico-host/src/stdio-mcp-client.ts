...
import { isWindows } from "@pico/runtime/host-shell";
...
} from "@pico/runtime/sensitive-data-redaction";
import { managedProcessLauncher } from "./process-sandbox/index.js";
...
export class StdioMcpClient implements McpClient {
  ...
  async connect(): Promise<void> {
    ...
    this.diagnostics.info(
      { server: this.config.name },
      `[MCP] stdio server "${this.config.name}" 连接成功`,
    );
  }
  ...
  private resolvePending(response: JsonRpcResponse): void {
    ...
    if (!pending) {
      this.diagnostics.warn(
        { server: this.config.name, id },
        `[MCP] 收到未知 id=${id} 的响应,已忽略`,
      );
      return;
    }
  ...
  private notify(method: string, params: Record<string, unknown>): void {
    ...
    if (Buffer.byteLength(line, "utf8") > MAX_STDIO_MESSAGE_BYTES) {
      this.diagnostics.warn(
        { server: this.config.name, method },
        `[MCP] notification 超过大小上限,已忽略`,
      );
      return;
    }
    this.child.stdin.write(line);
  ...
  private notifyFlushed(method: string, params: Record<string, unknown>): Promise<void> {
    ...
    return new Promise((resolve) => {
      ...
      const timer = scheduleDeadline(finish, CANCELLATION_FLUSH_TIMEOUT_MS);
