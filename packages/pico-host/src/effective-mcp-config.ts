...
/** Data-only connection source shared with the outer MCP connection adapter. */
...
export interface EffectiveMcpSourceResolution {
  ...
  /** Collision-free inputs for the outer MCP connection manager. Plugin sources may be appended by the host. */
...
/** Apply host-first precedence before the outer Runtime connects to plugin sources. */
export function filterPluginMcpSources(
  ...
  return sources.flatMap((source) => {
    ...
    return Object.keys(mcpServers).length > 0 ? [{ ...source, config: { mcpServers } }] : [];
  });
}
