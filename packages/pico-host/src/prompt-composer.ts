...
/** Runtime-owned Goal state is rendered by Host without an Engine dependency. */
export interface PromptGoalManager {
  buildGoalContext(): string | undefined;
}

/** Host-owned inputs are expressed as the minimum prompt composition ports. */
export interface PromptSkillLoader {
  loadAll(): Promise<string>;
}

export interface PromptTodoStore {
  buildTodoContext(): Promise<string>;
}

export interface PromptComposerLogger {
  warn(bindings: object, message: string): void;
}

const NOOP_LOGGER: PromptComposerLogger = {
  warn: () => undefined,
};

export interface PromptComposerOptions {
  goalManager?: PromptGoalManager;
  todoStore?: PromptTodoStore;
  ...
  swarmMode?: boolean;
  /** Host structured logging; omitted for portable silent composition. */
  logger?: PromptComposerLogger;
  /** Allows alternative shell discovery without binding prompt policy to Engine. */
  hostShellDialect?: () => HostShellDialect;
}
...
export class PromptComposer {
  ...
  private readonly skillLoader: PromptSkillLoader;
  ...
  private readonly onInstructionsLoaded:
    | ((paths: readonly string[]) => void | Promise<void>)
    | undefined;
  /** 用户级 Pico Home（~/.pico），注入后加载用户级 AGENTS.md */
  private readonly picoHome: string | undefined;
  private readonly logger: PromptComposerLogger;
  private readonly getHostShellDialect: () => HostShellDialect;
  ...
  constructor(workDir: string, planMode = false, options?: PromptComposerOptions) {
    ...
    this.todoStore = options?.todoStore ?? new TodoStore(resolvePicoPaths(workDir).workspace.root);
    ...
    this.getHostShellDialect = options?.hostShellDialect ?? hostShellDialect;
  }
...
# 核心纪律 (CRITICAL)
1. 如需检查文件是否存在,请使用 bash 工具执行 ${this.isPowerShellHost() ? "Get-ChildItem 或 Test-Path" : "ls 或 test -f"},而不是对目录使用 read_file。
...
\`\`\``);
    ...
    if (!this.planMode) {
      try {
        ...
        this.logger.warn({ err }, "[composer] 构建 TodoList 上下文失败,降级跳过");
      }
    }
    ...
    try {
      ...
      this.logger.warn({ err }, "[composer] 构建 Goal 上下文失败,降级跳过");
    }
  ...
  /** 解析失败按 bash 回退，提示词保持保守且不阻断会话启动。 */
  private isPowerShellHost(): boolean {
    try {
      return this.getHostShellDialect() === "powershell";
    } catch {
      return false;
    }
  }
