...
import { startPicoDaemonRuntimeHostCandidate } from "./product-runtime-host-candidate.js";
...
async function isEntrypoint(moduleUrl: string): Promise<boolean> {
  const launched = process.argv[1];
  if (!launched) return false;
  try {
    return (await realpath(launched)) === (await realpath(fileURLToPath(moduleUrl)));
  } catch {
    return false;
  }
}

export async function runLocalDaemonEntrypoint(moduleUrl: string): Promise<void> {
  if (!(await isEntrypoint(moduleUrl))) return;
