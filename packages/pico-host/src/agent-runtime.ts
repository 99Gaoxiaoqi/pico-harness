...
import { sessionEntryKey } from "@pico/pico-host/session-manager-state";
...
import { SkillLoader, type Skill } from "@pico/pico-host/product-skill-catalog";
...
import type { CredentialResolver } from "@pico/pico-host/provider/credential-vault";
...
import type { DefaultToolRegistryOptions } from "@pico/pico-host/default-registry";
...
} from "@pico/pico-host/session-permissions";
import { bashCommandFromArgs } from "@pico/runtime/bash-paths";
import { computeApprovalDiff } from "@pico/pico-host/product-approval-diff";
import {
...
import { createSessionRuntime, type SessionRuntime } from "@pico/pico-host/session-runtime";
...
import { activatePluginProviderCapabilities } from "@pico/pico-host/plugins/plugin-provider-activation";
...
import { bindRuntimeHookCapabilities } from "@pico/pico-host/runtime-hook-assembly";
...
export { loadImage } from "@pico/pico-host/input/prepare-prompt";
...
export async function executeAgentRuntime(
  ...
  const picoHome = resolvePicoHome({
    ...(dependencies.picoHome !== undefined ? { picoHome: dependencies.picoHome } : {}),
  ...
  try {
    ...
    const reporter = dependencies.reporter ?? new SilentReporter();
