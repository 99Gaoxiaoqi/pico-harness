...
import { WORKSPACE_FILE_SIDE_EFFECTS } from "./tool-registry-contract.js";
...
function runForegroundCommand(
  ...
  return new Promise<ForegroundCommandResult>((resolvePromise, rejectPromise) => {
    ...
    let killTimer: ScheduledDeadline | undefined;
    let settled = false;
    ...
    const terminateWithGrace = (): void => {
      signalTree("SIGTERM");
      if (killTimer) return;
      killTimer = scheduleUnrefDeadline(forceKill, BASH_KILL_GRACE_MS);
    };
    ...
    const timeoutTimer = scheduleUnrefDeadline(() => {
      timedOut = true;
      terminateWithGrace();
    }, timeoutMs);
