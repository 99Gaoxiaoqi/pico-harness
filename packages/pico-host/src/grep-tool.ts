...
import { reportWorkspaceFileScans } from "@pico/runtime/file-scan-observer";
import {
  createSandboxPolicy,
...
export interface GrepToolOptions {
  ...
  readonly diagnostics?: GrepDiagnostics;
}

export interface GrepDiagnostics {
...
const NOOP_GREP_DIAGNOSTICS: GrepDiagnostics = {
  warn: () => undefined,
  debug: () => undefined,
};
...
async function collectFiles(
  ...
  signal?: AbortSignal,
  diagnostics: GrepDiagnostics = NOOP_GREP_DIAGNOSTICS,
): Promise<string[]> {
  ...
  async function walk(dir: string): Promise<void> {
    ...
    try {
      ...
      diagnostics.warn({ err, dir }, "grep 扫描子目录失败,跳过");
      return;
    }
...
async function searchWithNode(opts: {
  ...
  for (const rel of files) {
    ...
    try {
      ...
      opts.diagnostics.debug({ err, file: rel }, "grep 跳过不可读文件");
      continue;
    }
...
export class GrepTool implements BaseTool {
  ...
  private readonly diagnostics: GrepDiagnostics;
  ...
  constructor(
    ...
    this.roots =
      typeof workDirOrRoots === "string"
        ...
        : workDirOrRoots;
    this.diagnostics = options.diagnostics ?? NOOP_GREP_DIAGNOSTICS;
  }
    ...
    if (rgExecutable !== false && maxFiles === undefined) {
      try {
        const raw = await searchWithRg({
          ...
          lineNumber,
          maxResults,
          ...(excludeSensitiveFiles === undefined ? {} : { excludeSensitiveFiles }),
          ...(processSandbox
            ? {
                sandbox: {
                  ...processSandbox,
        ...
        rgExecutable = false;
        this.diagnostics.warn({ err }, "grep 的 rg 路径失败,降级到 Node.js 实现");
      }
    }
