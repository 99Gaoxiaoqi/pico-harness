...
export async function resolveProjectMcpConfigPath(
  ...
  if (picoConfig !== undefined) return { path: picoConfig, source: "pico", exists: true };
...
async function resolveSafeConfigFile(workspace: string, path: string): Promise<string | undefined> {
  ...
  if (entry.isSymbolicLink()) throw new Error(`项目 MCP 配置不允许使用符号链接: ${path}`);
  if (!entry.isFile()) throw new Error(`项目 MCP 配置必须是普通文件: ${path}`);
