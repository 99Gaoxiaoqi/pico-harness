import { subagentThinkingLevel } from "./configured-subagent-executor.js";
...
import { bindAgentGraphOperatorExecutionBoundary } from "./product-agent-graph-execution-boundary.js";
...
import type {
  ApprovalNotice as HostApprovalNotice,
  ApprovalNotifier as HostApprovalNotifier,
...
type ApprovalNotice = HostApprovalNotice<PermissionSessionScope>;
type ApprovalNotifier = HostApprovalNotifier<PermissionSessionScope>;
import { bashCommandFromArgs } from "@pico/runtime/bash-paths";
import { assertValidAgentGraphOperatorProfileSnapshot } from "@pico/runtime";
import { createCliSessionId, listCliSessionCatalogEntries } from "./session-resolver.js";
import { globalSessionManager } from "./session.js";
import type { SessionManagerLease } from "./session-manager.js";
import {
  AgentRuntime,
  type RunAgentCliResult,
  type RuntimeSessionResourceChangedNotice,
} from "./agent-runtime.js";
import { currentRuntimeRun, RuntimeRun } from "./product-runtime-run.js";
...
} from "./product-agent-graph-host.js";
...
import { SilentReporter } from "@pico/runtime";
...
import { resolveTrustedEffectiveMcpSources } from "./effective-mcp-config.js";
...
} from "./background-autonomous-policy.js";
...
import { hasExplicitNetworkIntent } from "./workspace-sandbox.js";
import { canonicalizeSandboxBoundaryExpansion } from "@pico/runtime/sandbox-boundary-path";
import { workspaceAccessesFromCall } from "./workspace-roots.js";
...
import { DesktopReporter, type DesktopReporterEvent } from "./index.js";
...
import { buildApprovalRequestedPayload } from "./index.js";
import {
...
import { LocalDaemonHost } from "./local-daemon-host.js";
...
import { AGENT_GRAPH_SUPERVISOR_TOOL_NAMES } from "./agent-graph-tools.js";
...
export function createProductionRuntimeServices(
  ...
  const picoHome = resolvePicoHome(
    suppliedEnv["PICO_HOME"] === undefined ? {} : { picoHome: suppliedEnv["PICO_HOME"] },
  );
  ...
  const refreshGraphOperatorCatalog = async (workspacePath: string) => {
    ...
    catalog.replaceProfiles(
      await loadAgentCatalog<HookTrustAuthority>({
        logger,
              ...
              ? await agentRuntime.approvePlanAndExecute(
                  {
                    approval: {
                      ...
                      ...(planTransitionOperationId === undefined
                        ? {}
