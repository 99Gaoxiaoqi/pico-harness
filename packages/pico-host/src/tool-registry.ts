// Host-owned Registry implementation: tool bindings, middleware ordering,
// Hook dispatch, schema validation and physical resource admission.
...
const NOOP_TOOL_REGISTRY_DIAGNOSTICS: ToolRegistryDiagnostics = {
  info: () => undefined,
  warn: () => undefined,
};
...
export class ToolRegistry implements Registry {
  ...
  constructor(
    private readonly resourceAuthority: Pick<
      ...
      "run"
    > = sharedToolResourceAuthority,
    private readonly diagnostics: ToolRegistryDiagnostics = NOOP_TOOL_REGISTRY_DIAGNOSTICS,
  ) {}
  ...
  setHookService(service: HookService): void {
    ...
    this.diagnostics.info("[Registry] 已挂载会话级 HookService");
  }
  ...
  register(tool: BaseTool): void {
    ...
    if (this.tools.has(name)) {
      ...
      this.diagnostics.warn({ tool: name }, `[Warning] 工具 '${name}' 已被注册,将被覆盖。`);
    }
    this.tools.set(name, tool);
    this.toolOwners.delete(name);
    this.diagnostics.info({ tool: name }, `[Registry] 成功挂载工具: ${name}`);
  }
  ...
  registerOwned(tool: BaseTool, owner: ToolRegistrationOwner): void {
    ...
    this.diagnostics.info(
      { tool: name, owner: `${owner.kind}:${owner.id}` },
      `[Registry] 成功挂载工具`,
    );
  }
  ...
  unregisterForHostPolicy(name: string): boolean {
    ...
    this.toolOwners.delete(name);
    if (removed) {
      this.diagnostics.info({ tool: name }, `[Registry] 已卸载工具: ${name}`);
    }
    return removed;
  }
  ...
  getAccesses(call: ToolCall): ToolAccesses {
    ...
    try {
      ...
      this.diagnostics.warn(
        { tool: call.name, err },
        `[Registry] accesses 声明失败,降级为 all() 保守`,
      );
        ...
        dispatchPromise = (async () => {
          ...
          if (this.preWriteHook) {
            try {
              ...
              this.diagnostics.warn(
                { err, tool: currentCall.name },
                "[Registry] preWriteHook 失败",
              );
            }
          }
          context?.signal?.throwIfAborted();
