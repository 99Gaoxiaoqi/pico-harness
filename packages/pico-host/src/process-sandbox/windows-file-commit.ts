...
export class WindowsFileCommitError extends Error {
  ...
  constructor(message: string, readonly outcomeUnknown: boolean, options?: ErrorOptions) {
...
export async function commitWindowsFile(input: WindowsFileCommitInput): Promise<void> {
  ...
  const trustedPaths = await Promise.all([broker, helper, node].map(async (path) => {
  ...
  for (const trustedPath of trustedPaths) {
    if (writableRoots.some((root) => isWithinRoot(root, trustedPath))) {
      throw new WindowsFileCommitError(
        `受信提交程序位于目标进程可写目录: ${trustedPath}`,
        false,
      );
    }
  }
  ...
  await new Promise<void>((resolvePromise, rejectPromise) => {
    const child = spawn(trustedPaths[0]!, [
      "--commit-file", "--node", trustedPaths[2]!, "--helper", trustedPaths[1]!,
      "--target", targetPath,
    ], {
    ...
    child.once("error", (error) => {
      finish(new WindowsFileCommitError("Windows Broker 无法启动文件提交", dispatched, { cause: error }));
    });
    child.once("close", (code) => {
      if (code === 0 && stdout.trim() === "OK") {
        finish();
      } else {
        finish(new WindowsFileCommitError(
    ...
    child.stdin?.once("error", (error) => {
      finish(new WindowsFileCommitError("Windows Broker 请求传输失败，结果未知", true, { cause: error }));
    });
    dispatched = true;
