...
export interface LspDiagnosticLogger {
...
const NOOP_LOGGER: LspDiagnosticLogger = {
  debug: () => undefined,
  warn: () => undefined,
};
...
export class StdioLspClient {
  ...
  constructor(
    ...
    private readonly logger: LspDiagnosticLogger = NOOP_LOGGER,
  ) {}
  ...
  private handleMessage(body: string): void {
    ...
    try {
      ...
      this.logger.warn({ server: this.config.id }, "[LSP] 忽略无法解析的 JSON-RPC 消息");
      return;
    }
