...
import { getSupportedToolNames } from "@pico/runtime/tool-surface";
...
import { ensureWorkspaceTrusted, WorkspaceTrustStore } from "../workspace-trust.js";
...
const pendingLockReleases = new Map<
  ExclusiveCaseLocks,
  { attempt: number; timer?: ScheduledDeadline | undefined }
>();
...
function createCancellation(
  ...
  const timeout = scheduleDeadline(() => cancel("timeout"), timeoutMs);
...
function resultPayload(input: {
  ...
  policyDenials?: HeadlessOneShotPolicyDenialSummary | undefined;
  handoff?: PlanHandoff;
...
function schedulePendingLockRelease(locks: ExclusiveCaseLocks): void {
  ...
  state.attempt++;
  state.timer = scheduleUnrefDeadline(() => {
