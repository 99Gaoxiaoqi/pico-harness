...
/** Minimal host-owned options needed when opening a durable operator Session. */
export interface AgentGraphOperatorSessionOptions {
  readonly picoHome?: string;
  readonly runtimeStorageRoot?: string;
}

/** Frozen operator settings relevant to execution-boundary inheritance. */
export interface AgentGraphOperatorSessionSettings {
...
export interface AgentGraphOperatorSessionRuntimeState {
  readonly boundary?: ExecutionBoundary;
  readonly settings?: AgentGraphOperatorSessionSettings;
}

export interface AgentGraphOperatorSession {
  getRuntimeStateSnapshot(): AgentGraphOperatorSessionRuntimeState;
...
export interface AgentGraphOperatorSessionLease {
  readonly session: AgentGraphOperatorSession;
  release(): void;
}

/** Engine lifecycle is injected by the application; Pico Host does not import Engine. */
export interface AgentGraphOperatorSessionManagerPort {
  getOrCreatePinned(
    id: string,
    workDir: string,
    options?: AgentGraphOperatorSessionOptions,
  ): Promise<AgentGraphOperatorSessionLease>;
}

export interface BindAgentGraphOperatorExecutionBoundaryInput {
  readonly sessionManager: AgentGraphOperatorSessionManagerPort;
  readonly rootSessionId: string;
  readonly childSessionId: string;
  readonly parentWorkDir: string;
  readonly childWorkDir: string;
  readonly workspacePolicy: "shared" | "isolated-worktree";
  readonly sessionOptions?: AgentGraphOperatorSessionOptions;
  ...
  }) => AgentGraphOperatorSessionSettings;
}
...
export async function bindAgentGraphOperatorExecutionBoundary(
  ...
  let childLease: AgentGraphOperatorSessionLease | undefined;
  try {
    ...
    let settingsToCommit: AgentGraphOperatorSessionSettings | undefined;
    if (initializedSettings) {
      if (existingSettings) {
...
function assertOperatorSettingsMatchProfile(
  actual: AgentGraphOperatorSessionSettings,
  expected: AgentGraphOperatorSessionSettings,
): void {
  if (
