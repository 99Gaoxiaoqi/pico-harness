...
import type { RuntimeSessionSelection } from "@pico/core";
...
} from "@pico/storage/session-summary";
import { StorageOperationJournal } from "@pico/storage/operation-journal";

export type CliSessionMode = "new" | "continue" | "resume" | "fork";
export type { CliSessionSummary, CliSessionHistorySource } from "@pico/storage/session-summary";

/** @deprecated Use the UI-neutral RuntimeSessionSelection from @pico/core. */
export type CliSessionSelection = RuntimeSessionSelection;
...
async function findLatestSessionId(
  ...
  return (
    await listCliSessionSummaries(workDir, {
...
async function indexForkTargetOperations(
  ...
  return new StorageOperationJournal({
    storageRoot: resolvePicoPaths(workDir, {
      ...(picoHome === undefined ? {} : { picoHome }),
    }).workspace.root,
  }).listForkTargets();
}
