...
export function ReviewPage() {
  ...
  const changes = preview
    ? (conversation?.changes ?? data.changes)
    : review?.key === reviewKey
      ? review.changes
      : [];
  const fingerprint = preview
    ? (conversation?.changeFingerprint ?? data.changeFingerprint)
    : review?.key === reviewKey
      ? review.fingerprint
      : undefined;
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void actions
      .queryReview(workspacePath, runId)
      .then((value) => {
  ...
  useEffect(() => {
    ...
    let disposed = false;
    void actions
      .queryReviewDiff(workspacePath, runId, selected.path)
      .then((value) => {
        if (disposed) return;
        ...
        setDiff({ key: reviewKey, path: selected.path, patch: value.patch, fingerprint });
      })
      .catch((cause: unknown) => {
        if (!disposed) setError(cause instanceof Error ? cause.message : String(cause));
      });
      ...
      {loading ? (
        ...
        <div className="review-layout">
          ...
          <section className="diff-workspace">
            ...
            {rewindOpen && (!rewindPreview || rewindPreview.key === reviewKey) && (
              <div className="rewind-panel">
                <div>
                  ...
                  <span>
                    <strong>
                      {rewindPreview
                        ? `将回退 ${rewindPreview.changeCount} 项更改`
                        : "回到最近检查点"}
                    </strong>
                    <small>
                      {rewindPreview
                ...
                {rewindPreview ? (
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (sessionRef)
                        void actions
                          .previewRewind(sessionRef)
                          .then((value) =>
                            setRewindPreview(value ? { key: reviewKey, ...value } : undefined),
                          );
                    }}
                  >
                    预览 Rewind
