...
const icon = (name) => {
  const icons = {
    ...
    settings:
      '<circle cx="12" cy="12" r="3"/><path d="M19 12a7 7 0 0 0-.1-1l2-1.5-2-3.4-2.4 1A7 7 0 0 0 14.8 6l-.3-2.6h-4L10.2 6a7 7 0 0 0-1.7 1.1l-2.4-1-2 3.4L6.1 11a7 7 0 0 0 0 2l-2 1.5 2 3.4 2.4-1a7 7 0 0 0 1.7 1.1l.3 2.6h4l.3-2.6a7 7 0 0 0 1.7-1.1l2.4 1 2-3.4-2-1.5a7 7 0 0 0 .1-1z"/>',
...
const changesInspector = (expanded = false) => `
  ...
  <section class="section"><div class="section-title"><span>文件</span><span>6 · +92 −28</span></div><div class="file-list">
    ${[
      ...
      .map(
        ([kind, path, stat, id]) =>
          `<button class="file-row ${state.selectedFile === id ? "active" : ""}" data-action="select-file" data-file="${id}"><span class="file-kind">${kind}</span><span class="file-path">${path}</span><span class="file-stat">${stat}</span></button>`,
      )
      .join("")}
  </div></section>
...
const inspector = (view, activeTab) => {
  const content =
    activeTab === "agents"
      ? agentsInspector()
      : activeTab === "changes"
        ? changesInspector(view === "task-review")
        : activeTab === "trace"
          ? traceInspector()
          : currentInspector(view);
  const defaultBadge =
    view === "task-review" ? "6 个文件" : view === "task-failed" ? "需要处理" : "执行现场";
  return `
...
const controlbar = (view) => {
  if (view === "task-review")
    return `<footer class="controlbar"><span class="control-meta">隔离 worktree · main@4f29c8a</span><div class="control-spacer"></div><button class="control-button" data-action="rewind">↶ Rewind</button><button class="control-button" data-action="request-changes">要求修改</button><button class="control-button primary" data-action="complete-review">批准并应用</button></footer>`;
  if (view === "task-failed")
...
const taskView = (view, params) => {
  ...
  const config =
    {
      "task-running": {
        title: state.paused ? "实现桌面端任务工作台" : "实现桌面端任务工作台",
        kicker: "TASK #184 · codex/desktop-shell",
        stateText: state.paused ? "已暂停" : "执行中 · 等待批准",
        tone: state.paused ? "" : "attention",
        status: state.paused ? "review" : "attention",
        timeline: runningTimeline(),
      },
      "task-review": {
        title: "增加 Rewind 检查点",
        kicker: "TASK #176 · pico/review-rewind",
        stateText: "等待审阅",
        tone: "",
        status: "review",
      ...
      "task-failed": {
        title: state.cancelled ? "任务已停止" : "连接项目 MCP Server",
        kicker: "TASK #169 · local/main",
        stateText: state.cancelled ? "用户停止" : "任务被阻塞",
        tone: "failed",
        status: "failed",
...
const customizeView = (params) => {
  ...
  return pageShell(
    "customize",
    `<header class="page-head"><div class="page-head-copy"><h1>扩展</h1><p>管理 Skills、MCP 和未来的 Plugins；项目来源只在 Workspace 信任后加载。</p></div><button class="secondary-button" data-action="reload-extensions">重新扫描</button></header>${tabs}${content}`,
  );
};
...
const settingsView = (params) => {
  ...
  if (tab === "permissions")
    ...
    content = `<section class="settings-section"><h2>桌面通知</h2><p>锁屏通知只显示任务名和风险摘要，不显示命令、路径或 Prompt。</p>${[
      ["等待批准", true],
      ["需要你的选择", true],
      ["任务完成", true],
      ["普通工具调用", false],
      ...
      .map(
        ([label, on]) =>
          `<div class="setting-row"><div class="setting-copy"><strong>${label}</strong><span>${on ? "点击后深链到对应任务" : "保持静默，只在应用内显示"}</span></div><div class="setting-control"><button class="toggle ${on ? "on" : ""}" data-action="toggle-generic" aria-label="切换${label}通知"></button></div></div>`,
      )
      .join("")}</section>`;
  else
    content = `<section class="settings-section"><h2>Providers</h2><p>API Key 只来自环境变量或系统凭证库，界面不回显秘密。</p><div class="table"><div class="table-row"><div class="table-main"><strong>zhipu</strong><span>OpenAI compatible · api.zhipu.example</span></div><div class="table-cell">ZHIPU_API_KEY</div><div class="table-state">已连接</div><button class="ghost-button" data-action="provider-diagnose">诊断</button></div><div class="table-row"><div class="table-main"><strong>anthropic</strong><span>Claude native · api.anthropic.com</span></div><div class="table-cell">系统凭证库</div><div class="table-state">已连接</div><button class="ghost-button" data-action="provider-diagnose">诊断</button></div></div></section><section class="settings-section"><h2>当前模型</h2><div class="setting-row"><div class="setting-copy"><strong>zhipu/glm-5.2</strong><span>131k context · reasoning · tool call</span></div><div class="setting-control"><button class="secondary-button" data-action="model-menu">切换模型</button></div></div><div class="setting-row"><div class="setting-copy"><strong>Vision</strong><span>Provider 未显式声明</span></div><div class="setting-control"><span class="table-state paused">未知</span></div></div></section>`;
  return pageShell(
    "settings",
    `<header class="page-head"><div class="page-head-copy"><h1>设置</h1><p>配置 Provider、权限、Usage 和通知；运行时决定仍由 Pico 引擎执行。</p></div></header>${tabs}${content}`,
  );
};
...
const onboardingView = (view) => {
  ...
  const steps = Array.from(
    { length: 5 },
    (_, index) =>
      `<span class="step ${index < stepIndex ? "done" : index === stepIndex ? "active" : ""}"></span>`,
  ).join("");
  let panel = "";
  if (view === "workspace")
...
const approvalModal = () =>
  modalShell({
    ...
    actions:
      '<button class="danger-button" data-action="deny-approval">拒绝</button><button class="primary-button" data-action="approve-once">批准并继续</button>',
  });
...
const askModal = () =>
  modalShell({
    ...
    actions:
      '<button class="ghost-button" data-action="cancel-answer">取消问题</button><button class="primary-button" data-action="confirm-answer">确认选择</button>',
  });
...
const rewindModal = () =>
  modalShell({
    ...
    actions:
      '<button class="ghost-button" data-action="close-dialog">取消</button><button class="primary-button" data-action="confirm-rewind">校验并 Rewind</button>',
  });
...
const stopModal = () =>
  modalShell({
    ...
    actions:
      '<button class="ghost-button" data-action="close-dialog">取消</button><button class="danger-button" data-action="confirm-stop">停止任务</button>',
  });
...
const notificationsModal = () =>
  modalShell({
    ...
    actions:
      '<button class="ghost-button" data-action="mark-read">全部已读</button><button class="secondary-button" data-action="close-dialog">关闭</button>',
  });
...
const commandModal = () =>
  modalShell({
    ...
    body: `<input class="field wide" name="command-search" autocomplete="off" aria-label="搜索命令" data-autofocus placeholder="搜索任务或页面…"><div class="choice-list">${[
      ["工作首页", "work/home"],
      ["新建任务", "work/home?dialog=new-task"],
      ["运行中任务", "work/task-running"],
      ["等待审批", "work/task-running?dialog=approval"],
      ["需要回答", "work/task-running?dialog=ask"],
      ["待审阅", "work/task-review"],
      ["Rewind 预览", "work/task-review?dialog=rewind"],
      ["失败恢复", "work/task-failed"],
      ["完成摘要", "work/task-completed"],
      ["Automations", "automations"],
      ["扩展", "customize"],
      ["设置", "settings"],
      ["首次启动", "onboarding/welcome"],
    ]
      .map(
        ([label, route]) =>
          `<button class="choice" data-route="${route}"><span class="choice-mark">→</span><span class="choice-copy"><strong>${label}</strong><span>#${route}</span></span><span>↵</span></button>`,
      )
      .join("")}</div>`,
  });
...
const automationModal = () =>
  modalShell({
    ...
    actions:
      '<button class="ghost-button" data-action="close-dialog">取消</button><button class="secondary-button" data-action="automation-draft">保存为停用草案</button><button class="primary-button" data-action="create-automation">确认并创建</button>',
  });
...
const genericInfoModal = (kind) => {
  const data = {
    tool: [
      "工具详情",
      "完整参数、输出顺序和 Trace ID",
      '<div class="kv-list"><div class="kv"><span>Tool Call ID</span><code>call_7F2A9</code></div><div class="kv"><span>工作目录</span><strong>pico-harness/</strong></div><div class="kv"><span>耗时</span><strong>12.04s</strong></div><div class="kv"><span>退出码</span><strong>running</strong></div><div class="kv"><span>Trace ID</span><code>span_8B12</code></div></div><div class="command-block" style="margin-top:12px">pnpm vitest run tests/integration/desktop-flow.test.ts\n\n✓ 5 tests passed\n◌ desktop shell renders approval state…</div>',
    ],
    agent: [
      "子代理详情 · Rewind 数据流",
      "独立上下文与工具输出不会混入主任务时间线。",
      '<div class="kv-list"><div class="kv"><span>模式</span><strong>explore · required</strong></div><div class="kv"><span>当前动作</span><strong>验证指纹冲突保护</strong></div><div class="kv"><span>进度</span><strong>58%</strong></div><div class="kv"><span>Token</span><strong>6.1k</strong></div><div class="kv"><span>Worktree</span><code>只读</code></div></div><div class="trace-list" style="margin-top:14px"><div class="trace-row"><span>10:16</span><span>读取 rewind coordinator</span><strong>ok</strong></div><div class="trace-row"><span>10:18</span><span>检查冲突分支</span><strong>ok</strong></div><div class="trace-row"><span>10:20</span><span>汇总发现</span><strong>running</strong></div></div>',
    ],
    workspace: [
      "切换 Workspace",
      "正在运行的任务仍绑定原 Workspace。",
      '<div class="choice-list"><button class="choice selected" data-action="close-dialog"><span class="choice-mark">PH</span><span class="choice-copy"><strong>pico-harness</strong><span>7 个任务 · 当前</span></span><span>✓</span></button><button class="choice" data-action="close-dialog"><span class="choice-mark">AH</span><span class="choice-copy"><strong>agent-harness-course</strong><span>2 个任务</span></span><span>→</span></button><button class="choice" data-route="onboarding/workspace"><span class="choice-mark">＋</span><span class="choice-copy"><strong>添加 Workspace</strong><span>选择并建立新的信任记录</span></span><span>→</span></button></div>',
    ],
...
const render = () => {
  ...
  if (dialog)
    requestAnimationFrame(() => app.querySelector("[data-autofocus], .modal button")?.focus());
};
...
document.addEventListener("click", (event) => {
  ...
  const handlers = {
    ...
    "approve-once": () => {
      closeDialog();
      showToast("已批准本次命令 · 决定已写入审计时间线");
    },
    "deny-approval": () => {
      closeDialog();
      showToast("已拒绝操作 · Pico 将调整执行方案");
    },
    "cancel-answer": () => {
      closeDialog();
      showToast("已取消问题 · Agent 收到 cancelled");
    },
    "confirm-answer": () => {
      const answer = app
    ...
    "select-task": () => {
      ...
      routeTo(
        `work/${target.dataset.view}`,
        target.dataset.dialog ? { dialog: target.dataset.dialog } : {},
      );
    },
    "inspector-tab": () => updateRouteParams({ tab: target.dataset.tab }),
    "select-file": () => {
      state.selectedFile = target.dataset.file;
      render();
    },
    rewind: () => updateRouteParams({ dialog: "rewind" }),
    "confirm-rewind": () => {
      closeDialog();
      showToast("指纹校验通过 · 已恢复代码和对话 · 原指令已回填");
    },
    stop: () => updateRouteParams({ dialog: "stop" }),
    "confirm-stop": () => {
      state.cancelled = true;
      state.selectedTask = "failed";
      routeTo("work/task-failed");
      showToast("任务已安全停止 · 文件变化保留");
    },
    pause: () => {
      state.paused = !state.paused;
      render();
      showToast(state.paused ? "已在安全边界暂停 · 现场与队列已保留" : "任务已继续运行");
    },
    steer: () => app.querySelector(".composer textarea")?.focus(),
    send: () => {
      const input = app.querySelector(".composer textarea");
      if (!input?.value.trim()) return showToast("请输入要补充的方向");
      input.value = "";
      showToast("Steer 已加入 · 将在下一模型边界生效");
    },
    "request-changes": () => {
      state.selectedTask = "desktop";
      routeTo("work/task-running");
      showToast("修改要求已加入下一轮");
    },
    "complete-review": () => {
      state.selectedTask = "done";
      routeTo("work/task-completed");
      showToast("应用预检通过 · 变更已合入目标分支");
    },
    retry: () => {
      state.selectedTask = "desktop";
      routeTo("work/task-running");
      showToast("已从失败步骤创建新的安全重试");
    },
    ...
    "create-automation": () => {
      closeDialog();
      showToast("自动化已创建并启用 · 下次明天 09:30");
    },
    "automation-draft": () => {
      closeDialog();
      showToast("已保存为停用草案");
    ...
    "toggle-automation": () => {
      ...
      render();
      showToast(state.automationEnabled ? "自动化已启用" : "自动化已停用");
    },
    "toggle-skill": () => {
      state.skillEnabled = !state.skillEnabled;
      render();
    },
    "toggle-mcp": () => {
    ...
    "toggle-generic": () => {
      ...
      showToast(target.classList.contains("on") ? "已启用" : "已停用");
    },
