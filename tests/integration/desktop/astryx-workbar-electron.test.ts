...
test(
  "Astryx 工作栏集成：待办修订、地址提交、终端输入与产物菜单",
  { skip: !process.env.PICO_TEST_ELECTRON, timeout: 60000 },
  async (t) => {
    const root = await mkdtemp(join(tmpdir(), "pico-astryx-workbar-"));
    t.after(() => rm(root, { recursive: true, force: true }));
    const repo = fileURLToPath(new URL("../../../", import.meta.url));
    await Promise.all([
      build({
        entryPoints: [join(repo, "tests/fixtures/astryx-workbar-electron.renderer.tsx")],
        bundle: true,
        platform: "browser",
        format: "iife",
        jsx: "automatic",
      ...
      build({
        entryPoints: [join(repo, "tests/fixtures/astryx-workbar-electron.ts")],
        bundle: true,
        platform: "node",
        format: "esm",
        packages: "external",
    ...
    assert.match(output, /ASTRYX_WORKBAR_ELECTRON_OK/);
    console.log(output);
  },
);
