...
test(
  "Desktop 运行态清理终态待处理请求并在信任后刷新项目且防止切换竞态",
  { timeout: 45_000 },
  async (t) => {
    const candidates = [
    ...
    const bundle = await build({
      entryPoints: [
        fileURLToPath(
          new URL("../../fixtures/desktop-runtime-recovery-browser.tsx", import.meta.url),
        ),
      ],
      bundle: true,
      write: false,
    ...
    const profile = await mkdtemp(join(tmpdir(), "pico-runtime-recovery-ui-"));
    ...
    try {
      ...
      assert.match(result, /^PASS: runtime pending reconciliation and trust recovery$/, result);
    } finally {
      clearTimeout(timer);
