...
test("Graph 看板容器区分普通聊天、历史 Graph 与真实查询错误", { timeout: 45_000 }, async (t) => {
  const candidates = [
  ...
  const bundle = await build({
    entryPoints: [
      fileURLToPath(new URL("../../fixtures/desktop-graph-board-browser.tsx", import.meta.url)),
    ],
    bundle: true,
    write: false,
    format: "iife",
  ...
  const profile = await mkdtemp(join(tmpdir(), "pico-graph-board-ui-"));
  ...
  try {
    ...
    assert.match(result, /^PASS: Graph board discovery and query failure recovery$/, result);
  } finally {
    clearTimeout(timer);
