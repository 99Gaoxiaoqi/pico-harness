...
test("Desktop and Runtime contracts no longer expose retired Discovery methods", () => {
  for (const method of [
    "discovery.start",
    "discovery.get",
    "discovery.resume",
    ...
    assert.equal((RUNTIME_METHODS as readonly string[]).includes(method), false);
    assert.equal((DESKTOP_RUNTIME_METHODS as readonly string[]).includes(method), false);
    assert.equal(isRuntimeMethod(method), false);
  }
});
...
test("runtime schema and config notifications advertise scoped capabilities without secret data", () => {
  assert.equal(DESKTOP_RUNTIME_SCHEMA_REVISION, 18);
  assert.equal(DESKTOP_RUNTIME_SCHEMA_CAPABILITY, "desktop-runtime-schema-v18");
  const ping = {
    pong: true,
