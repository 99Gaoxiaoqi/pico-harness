...
/**
 * 架构不变量测试：把 docs/history/architecture/19-concepts-map.md 的核心断言变成可运行检查。
 *
 * 这些测试冻结 pico 的 4 条设计原则在叙事态（最干净的一态，4/5）的落地，防止
 * 未来重构/补丁无意破坏。每条测试对应 19 文档的一处断言，改实现前必须同步改测试
 * ——测试就是原则的"牙齿"。详见 docs/history/architecture/20-architecture-audit-and-governance.md。
 */
...
test("事件账本无事件级 GC（append-only 不变量）", () => {
  ...
  const store = readSource("packages/storage/src/sqlite/sqlite-runtime-event-store.ts");
...
test("投影入口 RuntimeProjectionService 不直接持久化（无第二事实源）", () => {
  ...
  const projection = readSource("packages/runtime/src/runtime-projection-service.ts");
...
// --- 已知架构债（活体追踪器）-----------------------------------------------
// 以下测试追踪 docs/history/architecture/20 的 P0/P1 债。与 test.todo 不同，这些是**普通测试**：
