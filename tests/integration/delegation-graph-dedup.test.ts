...
test("DelegationManager rejects a duplicate graphWorkId dispatch while the first is in-flight", () => {
  const { manager, cleanup } = createDelegationManager();
  try {
    ...
    // 此处验证 dispatch 汇聚点用 durable lease 对同一 graphWorkId 的重复派发去重。
...
test("DelegationManager rejects a duplicate graphWorkId dispatch during the settle-chain window (status terminal, settleFinalized false)", async () => {
  ...
  // 与下游派发仍在飞行中。durable lease 在此窗口仍未 release（.finally 未跑）→ 拦截。
  ...
  try {
    ...
    // 仍为 false（settle 链被 settleGate 卡住）。
    ...
    // 此时 durable lease 仍未 release（.finally 未跑）——这是修复要覆盖的关键窗口。

    // 同一 graphWorkId 的并发派发必须被 durable lease 拒。冲突时返回持有者 ownerId。
    ...
    // 不同 graphWorkId 不受 lease 波及（按 resourceKey 严格相等去重），仍可正常派发。
    const other = manager.dispatch(immediateRunner, { graphWorkId: "work-settle-2" });
    assert.equal(other.status, "dispatched");

    // 释放 gate，让两条 settle 链收口，进程可干净���出。
    releaseSettle();
