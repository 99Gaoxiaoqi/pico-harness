...
const fullAccessBoundary = createBypassExecutionBoundary();

test("foreground subprocesses compile only from the durable execution boundary", () => {
  const descriptor = compileRuntimeProcessSandbox({
    collaborationMode: "agent",
    executionBoundary: managedBoundary,
  ...
  assert.deepEqual(
    compileRuntimeProcessSandbox({
      collaborationMode: "agent",
      executionBoundary: fullAccessBoundary,
      workspaceGeneration: 4,
      scratchRoot,
    }),
    {
...
test("session network grants are scoped, one-shot, and fully revocable", () => {
  ...
  assert.equal(grants.consumeNetworkAuthorization("session-a", workDir, "call-a", picoHome), true);
  assert.equal(grants.consumeNetworkAuthorization("session-a", workDir, "call-a", picoHome), false);
  assert.equal(grants.consumeNetworkAuthorization("session-b", workDir, "call-a", picoHome), false);
  ...
  assert.equal(grants.consumeNetworkAuthorization("session-a", workDir, undefined, picoHome), true);
  ...
  assert.equal(grants.consumeNetworkAuthorization("session-a", workDir, "call-b", picoHome), false);
});
