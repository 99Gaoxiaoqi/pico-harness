...
import type { ReplOptions } from "../../src/tui/repl.js";
import type { CliStartupSession } from "../../src/cli/session-args.js";

/**
 * 3-D Phase 4 默认切换：CLI 入口分派。默认走 daemon 瘦客户端；--local 过渡
 * 逃生门走进程内；会话旗标三式（-S/--continue/--fork）与 --graph 传递到
 * ClientReplOptions；缺口旗标（--mcp-config/--add-dir）显式提示不静默丢弃。
 */
...
function harnessWithRuntime(): DispatchHarness {
  ...
  const runtime: CliRuntime = {
    ...
    startTuiRepl: async (options) => {
      replCalls.push(options);
    },
...
test("cli dispatch: 默认（无旗标）走客户端瘦 TUI", async () => {
  ...
  assert.equal(harness.replCalls.length, 0, "进程内路径不再默认触达");
  assert.equal(harness.clientCalls[0]!.workDir, "C:\\ws");
  assert.equal(harness.clientCalls[0]!.sessionId, undefined, "新会话不带 sessionId");
});

test("cli dispatch: --local 过渡逃生门走进程内；--client 兼容仍是客户端", async () => {
  const harness = harnessWithRuntime();
  assert.equal(await harness.run(["--local"]), 0);
  assert.equal(harness.replCalls.length, 1);
...
test("cli dispatch: help/version 快速路径不起 TUI", async () => {
  ...
  assert.ok(harness.stdout.some((line) => line.includes("--local")));
  assert.equal(harness.clientCalls.length, 0);
