...
import type { SkillRegistry } from "../../src/memory/skill-registry.js";
import type { FTS5Store } from "../../src/memory/fts5-store.js";
...
describe("MemoryNudger", () => {
  ...
  beforeEach(() => {
    ...
    nudger = new MemoryNudger(
      skillRegistry as unknown as SkillRegistry,
      fts5 as unknown as FTS5Store,
    );
  });
  ...
  describe("触发周期", () => {
    ...
    it("支持自定义周期", async () => {
      // 每 5 轮触发一次
      const customNudger = new MemoryNudger(
        skillRegistry as unknown as SkillRegistry,
        fts5 as unknown as FTS5Store,
        { nudgePeriod: 5 },
      );
...
/** 测试辅助函数：创建 mock 技能 */
function createMockSkill(name: string, successCount: number, failCount: number): LearnedSkill {
  return {
    id: `skill-${name}`,
    name,
    trigger: `触发 ${name}`,
