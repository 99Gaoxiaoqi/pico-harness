...
it("Stage 12 代码智能在真实临时仓库中完成 LSP 导航并确定性降级 Repo Map", async () => {
  ...
  const outsideDir = await mkdtemp(path.join(tmpdir(), "pico-stage12-code-outside-"));
  ...
  try {
    ...
    if (process.platform !== "win32") {
      ...
      await symlink(outsideSource, path.join(workDir, "linked.ts"));
      await expect(lsp?.diagnostics("linked.ts")).rejects.toThrow("越出工作区");
    }
    await writeFile(path.join(workDir, "oversized.ts"), "x".repeat(4 * 1024 * 1024 + 1), "utf8");
    await expect(lsp?.diagnostics("oversized.ts")).rejects.toThrow("字节上限");
    await expect(
      lsp?.definitions({ filePath: "index.ts", position: { line: 2, character: 24 } }),
    ).resolves.toMatchObject([
      {
        filePath: physicalSourcePath,
