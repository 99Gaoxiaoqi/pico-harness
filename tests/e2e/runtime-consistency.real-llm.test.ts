...
realModelTest(
  "large ToolResult persists full inline without Evidence (ADR 26 entry shaping)",
  { timeout: TEST_TIMEOUT_MS },
  async (context) => {
    const model = await configuredUserDefaultRealModel();
    const sandbox = await createSandbox("large-tool-result");
    context.after(() => cleanupSandbox(sandbox));
    const fileName = "large-inline.txt";
    const marker = `PICO_LARGE_EVIDENCE_${randomUUID().replaceAll("-", "").toUpperCase()}`;
    const sourceLines = Array.from(
      { length: 128 },
      (_, index) => `${String(index + 1).padStart(3, "0")}|全文内联😀|${"数据".repeat(35)}`,
    );
    ...
    assert.ok(expectedRawOutput.includes(marker));
    ...
    const result = await new AgentRuntime().execute(
      runtimeRequest(
        ...
        [
          ...
          "2. 从该工具返回的完整正文中找到以 PICO_LARGE_EVIDENCE_ 开头的 token。",
          "3. 最终只输出该 token，不要解释。",
        ].join("\n"),
        "new",
        ["read_file"],
    ...
    assert.equal(fileRead.result.refs.evidence, undefined);
    assert.equal(fileRead.result.data.body.storage, "inline");
    if (fileRead.result.data.body.storage !== "inline") {
      assert.fail("large read_file result must stay inline");
    }
    assert.equal(fileRead.result.data.body.content, expectedRawOutput);
    assert.equal(fileRead.result.data.body.sha256, expectedHash);
    assert.equal(fileRead.result.data.body.sizeBytes, expectedBytes);
    assert.deepEqual(fileRead.result.data.projection, {
      version: 1,
      mode: "full",
      text: expectedRawOutput,
      strategy: "original",
      truncated: false,
    });

    // 无 blob 写:工具结果路径不再产生 Evidence blob 目录。
    const evidenceBaseDir = resolvePicoPaths(sandbox.workDir, {
      picoHome: sandbox.picoHome,
    }).workspace.evidence;
    assert.equal(existsSync(join(evidenceBaseDir, "blobs")), false);
  },
);
