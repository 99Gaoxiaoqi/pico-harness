...
realModelTest(
  ...
  async () => {
    ...
    try {
      ...
      assert.deepEqual(
        planningTools.map((event) => event.data.toolName),
        ["read_file", "submit_plan"],
      );
...
function planningTask(): string {
  const commands = ["a", "b"].map((branch) => ({
    kind: "add",
    operator: {
      operator_id: `operator-${branch}`,
      generation: 1,
      role: "evidence reader",
      description: `Read the independent branch ${branch} evidence`,
      profile: { profile_id: "explore" },
      workspace: { kind: "shared" },
    },
    intent: {
      intent_id: `read-${branch}`,
      instruction: `Call read_file for branch-${branch}.txt. Then call agent_output exactly once with status success and output equal to that file's single line, with no newline or commentary. Do not edit files or use other tools.`,
      input_record_ids: [],
    },
  }));
  const overview = [
    "After approval, investigate two independent evidence files using exactly two explore Graph operators. The root must not read branch files itself.",
    "Mark both steps in_progress with update_plan; then call update_agent_graph once with this exact argument:",
    JSON.stringify({ expected_revision: 0, operation_id: "add-plan-branches", commands }),
    "Next call yield_agent_graph immediately. Its success ends this Run. Do not mark either step completed in the initial Run.",
    "On every Graph Supervisor wake, first call view_agent_graph. If an operator still runs, call yield_agent_graph and end that Run. Do not add or reactivate operators.",
    "Once both operators have successful records and terminal runtimeClaims, summarize both evidence values, then finish Graph with update_agent_graph using its current revision and a finish command selecting both record IDs from view_agent_graph.",
    "Only after Graph finish succeeds, call update_plan for step-a completed and step-b completed. End after execution is completed. Never cancel the plan or submit a second proposal.",
  ].join("\n");
  return [
