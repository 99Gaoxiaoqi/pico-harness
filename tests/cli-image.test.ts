// 5.5e 图片入口测试:CLI --image 路径 → loadImage 转 ImagePart。
//
// 验证:
...
import { loadImage, runAgentFromCli } from "../src/cli/run-agent.js";
...
describe("loadImage (CLI --image)", () => {
  it("读取 png 文件 → base64 + mimeType=image/png", async () => {
    ...
    try {
      ...
      const img = asBase64(loadImage(p));
      expect(img.type).toBe("image_base64");
  ...
  it("不同后缀推断不同 mimeType", async () => {
    ...
    try {
      ...
      for (const [name, expectedMime] of cases) {
        ...
        expect(asBase64(loadImage(p)).mimeType).toBe(expectedMime);
      }
    } finally {
      await safeRm(workDir);
    }
  ...
  it("大写后缀也能识别", async () => {
    ...
    try {
      ...
      expect(asBase64(loadImage(p)).mimeType).toBe("image/png");
    } finally {
      await safeRm(workDir);
    }
  });
...
describe("runAgentFromCli 带 imagePath", () => {
  it("imagePath → provider 收到的 user 消息含 images", async () => {
    ...
    try {
      ...
      const imagePath = await writeTmp(workDir, "snap.png", "PIC");
