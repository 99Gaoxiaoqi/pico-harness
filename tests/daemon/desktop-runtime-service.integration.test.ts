...
describe("DesktopRuntimeService integration", () => {
  ...
  it("仅在信任工作区安全初始化入口文件，并提供只诊断不修复的 Doctor 结果", async () => {
    ...
    await mkdir(diagnosticPaths.workspace.root, { recursive: true });
    await writeFile(diagnosticPaths.workspace.runtimeDatabase, "not-a-sqlite-database");
    ...
    expect(diagnostics).toMatchObject({
      ...
      checks: expect.arrayContaining([
        ...
        expect.objectContaining({ id: "runtime-ledger", status: "error" }),
  ...
  it("以 Session 为主体幂等发送多轮消息，并从 RuntimeEvent 分页恢复可见 Transcript", async () => {
  ...
  it("关闭 Desktop Runtime 时持久化活动 Run 的取消边界", async () => {
    ...
    const entries = await eventStore.readSessionEntries(sent.session.sessionId);
    const transcriptEvents = entries.flatMap(({ event }) =>
      event.kind === "transcript.event.recorded" ? [event.data.event] : [],
    );
    expect(transcriptEvents).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: "entry.appended",
          entry: expect.objectContaining({ kind: "run-boundary", status: "cancelled" }),
        }),
      ]),
    );
  });
