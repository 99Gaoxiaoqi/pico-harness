...
## 2026-09-15 源码组织收口与实际验收（进行中）

此前为渐进迁移保留兼容树；本轮按用户要求结束过渡期，不再维护旧内部 `src/*` 导入路径。
收口验收：根 `src` 仅四个进程入口和说明；消费者直接引用正式包；必要测试装配留在测试目录，
不复制整棵兼容树、不改变业务行为；架构门禁阻止旧结构回流。

- [ ] 测试消费者迁移，显式处理有默认值/路径/日志/类型语义的适配。
- [ ] 工程脚本、包验收与 Terminal-Bench 使用正式包入口。
- [ ] 删除旧根源码兼容树，更新当前模块地图并加入结构门禁。
- [ ] 集成构建、类型、架构与全量本地回归验证。
- [ ] 实际 Desktop package/make/隔离窗口验收与远端 CI 结果核实。
- [ ] 集成、推送及本轮临时工作区/旧远端任务分支收尾。
