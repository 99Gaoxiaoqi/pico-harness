...
## 2026-09-15 源码组织收口与实际验收

此前为渐进迁移保留兼容树；本轮按用户要求结束过渡期，不再维护旧内部 `src/*` 导入路径。
收口验收：根 `src` 仅四个进程入口和说明；消费者直接引用正式包；必要测试装配留在测试目录，
不复制整棵兼容树、不改变业务行为；架构门禁阻止旧结构回流。

- [x] 305 个测试文件直接消费正式包；路径、投影器、Hook 工厂与定价目录显式注入，
      未新增 `tests/support` 或另一棵兼容目录。
- [x] 工程脚本、包验收与 Terminal-Bench 使用正式包入口。
- [x] 删除 495 个旧根源码文件，保留 4 个进程入口和说明；README、ARCHITECTURE 与当前
      架构导航同步收口，冻结可视化产物不变。
- [x] 架构门禁禁止根 `src` 导入、非入口文件、生产引用测试以及可静态识别的旧源码路径拼接。
      全量发现的 Graph 跨进程 fixture 路径遗漏已修复并纳入门禁。
- [x] 冷构建、根/桌面类型、全量格式与 ESLint/架构检查通过；最终 350 个集成文件，
      1771 项中 1759 通过、12 项平台相关跳过、0 失败、0 取消。
- [x] 独立 npm pack/离线安装后 CLI 帮助/版本、两个 Headless 的单行 JSON/退出码与八包
      exports 通过；根 dist 仅包含 CLI、daemon 与 internal 入口产物。
- [x] 实际 Desktop package/make 生成 arm64 应用、ZIP/DMG；修复主进程 bundle 的
      `createRequire(import.meta.url)` 地址丢失，隔离窗口加载新任务页并成功连接 Runtime。
- [x] 核实上轮远端 CI：原生沙箱/平台任务通过，格式失败已修复；审批超时改用可取消的
      Runtime ref deadline，Code Mode 并发预算测试改为确定性的在途请求屏障。
      Desktop CI 路径触发覆盖全部 workspace 包。

本轮移除 8 条只比较旧兼容导出身份的测试，保留/增强实际工具注册执行验证，并增加源码结构、
Desktop bundle 启动和审批等待生命周期回归。前文阶段验收数字属于各自历史快照。
远端最终验收以本次提交后的 GitHub Actions 结果为准，不用本地结果代替平台执行。
...
- 渐进迁移阶段曾保留旧 import 路径；本轮按用户要求结束过渡期，内部消费者使用正式包导出。
