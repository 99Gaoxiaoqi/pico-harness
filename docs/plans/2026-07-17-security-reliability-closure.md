...
> 状态：本地与 Linux 矩阵完成，等待 Windows CI
> 建立日期：2026-07-17
...
- Node 未提供覆盖当前实现所需的 `openat/renameat/linkat` dirfd/匿名 inode 发布组合，文件发布通过重复 realpath、目录 inode 和版本复核缩小但不能从 OS 层消除同一用户在最后复核后替换父目录、目标路径或临时路径的窗口。具名临时 inode 恢复最终权限后，最终被授权的目录扫描者还可能在极短的 rename 前窗口看到它。
...
- YOLO hardline 是可见命令文本和已建模执行入口的保守拒绝器，不是任意 executable 的 OS 能力沙箱；少量无法证明安全的 Shell 启动配置会按设计产生 fail-closed 误拒绝。
...
- [x] 所有已确认问题有实现引用和回归证据。
- [ ] 每批集成检查通过，最终验证矩阵无未说明失败（等待 Windows CI）。
- [ ] 独立复审无未解决的高/中风险问题。
