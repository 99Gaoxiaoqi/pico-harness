...
| `/rename <title>`              | 重命名当前 Session；`idle`                                       | 点击会话标题或标题菜单“重命名”                                                                                  | 主路径   | 空标题拒绝；成功后侧栏、标题栏和 RuntimeEvent 投影同步更新                                  |
...
| `/plugin`               | 不提供公开安装/市场 UI；已安装且受信的资源型 Plugin 只由 Runtime snapshot 供内部 Skills/Agents 等能力使用 | 高级入口 |
