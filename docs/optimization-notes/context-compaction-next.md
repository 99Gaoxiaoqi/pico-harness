...
- `TTL`: 默认 168 小时,可按写入请求覆盖。
- `quota`: 默认总量 200MiB,达到 quota 时优先清理最旧的未 pinned artifact。
- `pinned`: 错误输出默认 pinned,避免 cleanup 把关键失败证据删掉。
- `metadata`: 每个 artifact 记录 tool name、args hash、createdAt、sizeBytes、ttlHours、summary、pinned 和 path。
- `opportunistic cleanup`: 写入新 artifact 后顺手 cleanup,不要求每轮都做全量扫描。
