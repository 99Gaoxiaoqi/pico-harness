...
> 文档状态：部分过期。工具披露以[决策记录 23](../decisions/23-decision-tool-disclosure-surface.md)
> 为准；ToolResult/Evidence 部分以[决策记录 26](../decisions/26-decision-tool-result-entry-shaping.md)
...
2. **注意力被稀释**——模型在海量无关工具和冗长输出里寻找正确选项,命中率下降。
