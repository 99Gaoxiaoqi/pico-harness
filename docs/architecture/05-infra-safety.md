...
| 方案             | 文件                    | 依赖     | 状态                                                  |
| ---------------- | ----------------------- | -------- | ----------------------------------------------------- |
| git stash 快照   | `checkpoint-manager.ts` | git 仓库 | legacy/manual fallback（保留不删，非公开 TUI 主路径） |
...
3. **双轨安全**：checkpoint-manager（legacy/manual git fallback）+ file-history（纯 copyFile 现行主方案），三轴 rewind 在 Session 层组合
