...
| 4. Skills 清单    | 项目与用户 Pico Skill Catalog 元数据（仅 name + description） | 至少一个技能          |
...
- 持久化到 `$PICO_HOME/workspaces/<workspace-id>/memory/skills/`
...
- **连接池化**：同一 Pico workspace 的多 Session 共享一个实例
- WAL 模式：并发读写 + 断电恢复
