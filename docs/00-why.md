...
**第二，JSON 处理是刚需。** Agent 与模型之间的通信全是 JSON——ToolCall 参数是 JSON，Provider 响应解析是 JSON，RuntimeEvent 也以结构化 JSON payload 写入本地 JSONL。Go 的 `encoding/json` 需要定义 struct，TypeScript 的 `JSON.parse` 配合 `interface` 零摩擦。
