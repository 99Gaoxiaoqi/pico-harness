...
**死锁取证方法论（stderr 基础设施的直接回报）**：真 home 的 candidate-logs 在 e2e 失败窗口抓到 4 个候选全被"旧版本 daemon 在运行"拒绝（实为劣化 daemon 持锁 + 重生被阻断）；测试 root 的 candidate-logs 抓到 `EPERM rename registration.json` 崩溃栈——两处都是零改动收益（候选输出自动落盘）。诊断脚本保留 `scripts/diagnostics/stress-daemon-diag.ts`（死端点高频压测 + pid 时间线 + 证据倒出）与 `scripts/diagnostics/probe-daemon.ts`（kernel/业务双层探针）。
...
**注意**：e2e 不在任何 CI 门内（RUN_LLM_E2E 手动）——曾按评审建议落地每日定时 workflow（`4ae5b9e2`），2026-08-17 用户拍板撤销（模型端点为内网地址，GitHub Actions 不可达，撤销原因见文末"已知边界"段）。
...
- **3-D 客户端已知边界（2026-08-16 渐进项收口后仍接受项）**：~~keybindings 与 @文件补全未接~~（已接：loadPicoConfig→App.keybindings + FileIndex fileMention）；~~argumentCompleter 异步补全~~（已接：/resume /fork /skill /agent RPC 候选 + TTL 缓存）；~~/changes 单文件恢复无协议对应~~（已闭环：rewind.changes/restoreFile + ChangesDialogHost，e2e 场景 3 实测）；~~provider/cron/usage 镜像与 memory remember~~（已镜像：memory.create 新协议 + provider._ + jobs._；cron add/credential 因 automation.create 凭据门与 provider default clear 明确降级提示）；~~仍 BLOCKED：mcp 控制面（reload/enable/...无 RPC）、context（本地引擎态）、operations/snapshots/add-dir/plugin/hooks~~（已全量收口 2026-08-16：mcp.user.setEnabled/session.context.get/session.directories.add/hooks.manage/operations.manage/plugin.manage + /snapshots 纯镜像，parity 豁免表清零；reload/活连接类子命令与 cron add 同款明确降级提示）；~~仍开放漏账两项（2026-08-16 盘点定性，未做也未正式接受）：attachments 输入仍忽略、审批 wire 缺 diff/sessionScope~~（已闭环：attachments 协议参数门+daemon commit+客户端接入，e2e 场景 7；审批 wire 三字段+parseApprovalSessionScope+allow_session 授权复用，e2e 场景 6）；~~**e2e CI 定时门**~~（2026-08-17 撤销：`claude.jlcops.com` 公网 DNS 解析到内网 10.x IP，GitHub Actions 服务器物理不可达——即使配齐 Secrets 也只会天天红灯；`4ae5b9e2` 落地的 workflow 文件按用户拍板删除，e2e 维持 `RUN_LLM_E2E` 本机手动跑）；client-session-runtime 位置在 src/tui 接受（TuiReporter 端口耦合）。
