...
> [`18-graph-mode.md`](../../architecture/18-graph-mode.md)。
...
Graph v2 由 `orchestrationMode="graph"` 开启，根 Agent 使用 `view_agent_graph`、`update_agent_graph`和 `yield_agent_graph`；Operator 使用 `agent_output` 提交结果。调度权威是 workspace `pico.sqlite` 中的 `agent_graph_*` 表，执行权威仍是普通 RuntimeEvent ledger。具体领域契约、Claim、exact Run 和 yield/wake 恢复见 [`18-graph-mode.md`](../../architecture/18-graph-mode.md)。
