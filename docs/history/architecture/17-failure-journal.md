...
> 文档状态：原子记忆迁移前的研究快照，不定义当前行为。Pico 现已使用六类原子 Item
> （含 `failure`），旧四类 Fact/Proposal/Worker 对照已过期；这仍不等于已实现失败 Run 自动
> 蒸馏。当前机制见[原子长期记忆](../../architecture/14-workspace-memory.md)。外部项目比较保留其研究时点，
> 不作为最新状态结论。
