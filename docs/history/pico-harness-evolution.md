...
> 这是一份项目历史与设计决策档案，不是 Agent 的常驻待办，也不要求新会话默认阅读。
> 这里的勾选表示当时的交付状态；历史上的产品边界、入口和取舍仅用于理解上下文，不构成当前约束。

## 如何阅读

- 想快速了解项目：先读“演进总览”和“阶段交付一览”。
- 想追溯某个模块：从对应阶段进入，随后查看“变更记录”中的实现背景与验证证据。
- 想规划新工作：另建短小、可完成的计划；只有新工作明确承接某一阶段时，才更新该阶段或新增篇章。

## 演进总览

| 时期               | 核心问题                                               | 代表阶段         | 留下的能力与思考                                             |
| ------------------ | ------------------------------------------------------ | ---------------- | ------------------------------------------------------------ |
| 基础可用性         | 让最小 Harness 真正可用                                | 1、1.5           | 流式输出、审批、MCP、文件历史与三轴 rewind                   |
| 工具与控制流       | 扩展 Agent 的行动和长程控制能力                        | 2、3             | 工具生态、后台任务、上下文压缩、steer、goal、plan review     |
| 模型与交互探索     | 验证多模型、多入口与 Claude Code 式交互                | 4、5、5.1、6、7  | Provider 路由、任务运行时、命令与 TUI 体验；部分外壳后来退役 |
| 可靠运行时         | 让主会话、worker、存储和可观测性可恢复、可治理         | 8–16、12.5、12.6 | 模型能力、隔离执行、MCP 生命周期、事件溯源、存储治理         |
| 下一篇章（待设计） | 从现有应用中提取可复用 runtime，为桌面端和手机端做准备 | —                | 先厘清 runtime 与平台 adapter 的边界，再单独记录计划与验收   |

## 当时采用的工程方法（归档）

这轮演进中曾采用“集成主链验证、小步提交、必要时以 worktree 并行、阶段完成后记录证据”的方法。它帮助了大规模并行开发，但不是所有后续任务的默认流程；当前执行规则以 `AGENTS.md` 为准。
...
> 这是阶段完成时的快照。130 项历史任务均已收口；它不代表当前或未来的产品范围。
...
| 阶段 6       | 8        | 8       | ✅ 完成                                  |
| 阶段 7       | 8        | 8       | ✅ 完成                                  |
| 阶段 8       | 8        | 8       | ✅ 当时的 TUI-only 收口完成（现为历史）  |
...
| **历史总计** | **130**  | **130** | ✅ 当时已排期任务全部完成                |

---

## 未归档候选与观察

> 这里保留当时发现、但未纳入阶段交付的候选事项与问题记录。它们不是默认待办；重新启动前应先结合当前 runtime 方向判断是否仍有价值。
