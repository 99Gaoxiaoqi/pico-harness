...
确定性的参数审计拒绝属于派发前准入：不创建该子调用的 T1 / operation，记录普通 rejected 结果，允许父 exec 正常结算并持久化 Turn 终态。只有受信审计代码在访问存储前产生的拒绝可以走此路径；真实 T1 / T2 存储异常仍失败关闭。后续子调用被拒绝不会抹去同一 Cell 中先前已完成操作的副作用。
...
普通对话不通过自然语言关键词强制首轮启动子 Agent，不据此隐藏工具、注入隐藏指令或生成重试。模型根据原始用户请求选择是否调用 `agent_spawn` 或 Graph 工具；显式 Plan / Graph / Swarm 模式约束，以及工具的 Schema、权限、预算和完成协议仍由宿主执行。
