...
| 维度       | pico                                 | Claude Code        |
| ---------- | ------------------------------------ | ------------------ |
| 完成判定   | LLM 评估器（≥3 轮无进展时触发）      | `/goal` 外部评估器 |
| 停滞检测   | 工具调用指纹 + 阈值梯度              | Stop hook 8 次拦截 |
| 延续协调器 | 分层续行（直接续行 / 评估器 / 终止） | `/goal` 续行       |
| 预算维度   | 轮次 / token / 成本 / 墙钟           | —                  |
| 预算提醒   | 剩余 ≤20% 时显示 ⚠                   | —                  |
...
**关键**：指纹只看 `工具:参数`，不看 `toolCall.id`。所以即使模型每轮生成不同的 `call-1`, `call-2`，只要工具和参数一样就算停滞。
...
评估器超时或调用失败时返回 `evaluatorFailed: true`，延续协调器看到这个标记直接允许退出（不阻塞）。
